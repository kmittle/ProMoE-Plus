...
| `scripts/MoS_repa/` | MoS, MoS Naive, MoS Naive Choice (block-range sweep, Sep, Blockwise, PerBlock, Fused, Shared-Align, RMSNorm, No-Coeff, proj_coeff sweep), Multi-Align (including TAR and SRSR), Cross-Attention MoS, L/XL scale-up |
...
| Multi-Align | `_repa_multi_align.py`, `_repa_multi_align_affinity.py`, `_repa_multi_align_spectral.py` | `train_with_MoS_repa.py` | Per-token sigmoid coefficients from `AlignCoefficientPredictor`. Config ablation flag: `use_dynamic_coeff` (`True` default / `False` removes predictor, uses uniform weighting). The affinity sibling adds parameter-free Teacher-Affinity Routing (TAR); the SRSR sibling adds a training-only shared-low/routed-high DINO responsibility loss and a reverse control. |
