...
| `scripts/MoS_repa/` | MoS, MoS Naive, MoS Naive Choice (block-range sweep, Sep, Blockwise, PerBlock, Fused, Shared-Align, RMSNorm, No-Coeff, proj_coeff sweep), Multi-Align (including TAR, SRSR, and TCEG), Cross-Attention MoS, L/XL scale-up |
...
| Multi-Align | `_repa_multi_align.py`, `_repa_multi_align_affinity.py`, `_repa_multi_align_spectral.py`, `_repa_multi_align_expert_geometry.py` | `train_with_MoS_repa.py` | Per-token sigmoid coefficients from `AlignCoefficientPredictor`. Config ablation flag: `use_dynamic_coeff` (`True` default / `False` removes predictor, uses uniform weighting). TAR matches teacher/router affinities; SRSR assigns low/high teacher components to shared/routed branches; TCEG matches the centered cosine geometry of routed-expert output centroids to teacher centroids and uses a fixed spatial teacher roll as its negative control. |
...
- **ProMoE-Multi-Align**: Returns `(pred, repa_loss)` during training. Similar to MoS-REPA but aligns with teacher last layer only; `AlignCoefficientPredictor` produces per-token sigmoid coefficients that weight the alignment loss. When `use_dynamic_coeff=False`, the predictor is removed and alignment uses uniform weighting (plain mean of negative cosine similarity). The TAR, SRSR, and TCEG siblings return a third `teacher_affinity_loss`, `spectral_responsibility_loss`, or `expert_geometry_loss`. The outer loop dispatches that value by registered family and weights it with top-level `teacher_affinity_coeff`, `spectral_responsibility_coeff`, or `expert_geometry_coeff`; at most one may be positive in an experiment arm. All are trained via `train_with_MoS_repa.py`.
