...
- **Run-time wrappers (`scripts/_run_times/<date>/`)**: experiments are launched through thin per-date wrapper scripts that **claim their GPUs at launch time**, so the same script runs on a 4-GPU machine and an 8-GPU machine without editing anything. Two-step flow whenever you "write an experiment script":
  1. Create the semantic script `scripts/<family>/run_<...>.sh` as usual (template.sh-based) — unchanged from before.
  2. Write the run-time wrapper with `scripts/_run_times/new_run.sh --script scripts/<family>/run_<...>.sh --gpus N [--date YYYY_MM_DD] [--desc <name>] [--dry-run]`. `--gpus N` is how many GPUs this experiment uses for DDP (any positive integer). It writes a placeholder `gpu_ids` of the first N ids into that experiment's YAML and creates `scripts/_run_times/<date>/<desc>.sh`, a wrapper that claims N idle GPUs at launch and `exec`s the semantic script.
  - **No fixed GPU ids, no slot numbers** (rule changed 2026-09-23, when 4-GPU machines entered the pool). The old scheme bound `X.1` to GPU `0-3`, `X.2` to `4-7` and so on, which assumed every machine had 8 GPUs. Wrappers are now named `<desc>.sh` with no slot prefix, and at launch they source `scripts/_run_times/claim_gpus.sh` to take the first N idle GPUs in id order, handing them to the semantic script through `PROMOE_GPU_IDS_OVERRIDE`.
  - **What "idle" means**: used memory below `PROMOE_GPU_IDLE_MAX_MIB` (default 100, which ignores the few MiB of driver residue an unused card reports) and no live claim from another launch. Claims are PID files under `/tmp/promoe_gpu_claims`, taken under one `flock` and released on exit; a claim whose PID is gone is stale and gets reused. They cover the minute between choosing a GPU and the trainer actually allocating on it, when a second launch would otherwise see the same zeros. **Not enough idle GPUs is a hard failure** -- the wrapper prints every GPU's state and exits rather than queueing.
  - **`gpu_ids` in the YAML is a placeholder**: `new_run.sh --gpus N` writes the first N ids, which fixes the DDP world size and keeps the semantic script runnable on its own; the wrapper overrides the actual ids every launch. `PROMOE_GPU_IDS_OVERRIDE` set by hand still wins over the claim, which is how you pin an experiment to particular cards.
  - **Every arm of one comparison must use the same `--gpus`.** Per-rank quantities -- routing-contrastive class centers, pooled expert representations, EC-BC selection -- change with the world size, so arms at different GPU counts are not comparable, and neither is a 2-GPU arm against the 4-GPU canonical baseline.
  - **Scope = one date dir only**: `--date` (default today, `YYYY_MM_DD`) just groups a batch; it no longer carries GPU meaning. `--desc` defaults to the semantic script's distinguishing name (`run_B_xxx_train_sample_eval.sh` -> `B_xxx`), and an existing wrapper of that name is an error rather than an overwrite. Use `--dry-run` to preview without writing.
  - **Wrappers written before 2026-09-23 keep their slot names and hard-coded `gpu_ids`** and are left alone; only new experiments use the claiming wrappers.
  - **Every date dir carries a `commands.md` command table** (user rule, 2026-09-14). The file holds **only the table** — no title, intro paragraphs or notes. It has the header `| 实验描述 | git分支 | 启动命令 | 输出位置 |` and one row per `<desc>.sh` wrapper, ordered as in the date dir, for example `| <中文实验描述>（N 卡） | repa | bash scripts/_run_times/<date>/<desc>.sh | outputs/<model_name>/<custom_cfg_name>/ |`. Date dirs from before 2026-09-23 instead lead each row with `Slot X.Y · GPU a-b ·`, since their wrappers still carry slot names. Every cell is plain text with no quotes or backticks, so it can be copied directly. `scripts/_run_times/2026_09_14/commands.md` shows the row format; copy its table, not its intro paragraphs. Update the table whenever a wrapper in that dir is added, renamed or removed. This is separate from the `/command-table` skill's `commands.csv`.
...
If the ablation is controlled by an existing config flag (e.g., `router_norm_type`, `align_target`, `use_dynamic_coeff`), only a new YAML config and shell script are needed — the `model_name` stays the same. Add the flag with a default that preserves backward compatibility. Create its run-time wrapper with `scripts/_run_times/new_run.sh` as well (see "Run-time wrappers").
...
6. **Run-time wrapper**: write it with `scripts/_run_times/new_run.sh --script scripts/<family>/run_<size>_<variant>_train_sample_eval.sh --gpus N` instead of hand-setting `gpu_ids`; the wrapper claims N idle GPUs at launch (see "Run-time wrappers" above).

## Important Notes
- **Experiments run on other servers; this server is primarily for writing code.** Training/sampling/evaluation are launched elsewhere — on this machine expect to author models, configs, scripts, and run-time wrappers, not to run real training. Experiment results (per-experiment FID/IS at 300K/500K, cfg 1.0/1.5) live on a dashboard at `https://video-generation-wulanchabu.oss-cn-wulanchabu.aliyuncs.com/yujie/projects/promoe_plus/index.html`. The **WebFetch tool is blocked** for this domain, but plain `curl` from the server reaches it fine — pull and parse locally: `curl -sS -L "<url>" -o /tmp/promoe_dashboard.html` then read `/tmp/promoe_dashboard.html` (the results are one HTML `<table class="cmp">`).
...
- Two skill directories mirror each other and must be edited together: `.claude/skills/` (Claude Code) and `.agents/skills/` (the active Codex definitions — `check-cc`/`inspect-cc` are the Codex-side names of `check-codex`/`inspect-codex`). `.codex/skills/` holds only `clear` and `init`, which have no counterpart in the other two, so it is not part of that mirroring.
