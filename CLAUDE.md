...
- `cfg.data_path` in `config.py` is the ImageNet train directory. It defaults to the repo-relative `datasets/imagenet/train` (where `prepare_imagenet.sh` materialises the data) and is overridable via the `PROMOE_DATA_PATH` env var; train.py has no `--data-path` CLI flag. Kept relative on purpose — train.py derives latent paths with `str.replace('train', ...)`, which requires `train` to appear exactly once in the path. See "Dataset auto-preparation".
