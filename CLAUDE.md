...
| Multi-Align | `_repa_multi_align.py`, `_repa_multi_align_affinity.py` | `train_with_MoS_repa.py` | Per-token sigmoid coefficients from `AlignCoefficientPredictor`. Config ablation flag: `use_dynamic_coeff` (`True` default / `False` removes predictor, uses uniform weighting). The affinity sibling adds parameter-free Teacher-Affinity Routing (TAR) supervision to one MoE router during training. |
