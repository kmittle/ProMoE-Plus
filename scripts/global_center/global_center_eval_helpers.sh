...
# Evaluate one checkpoint's generated images for the global-center run.  The
...
if [[ -n "${REPO_ROOT:-}" && -f "${REPO_ROOT}/scripts/_eval_metric_helpers.sh" ]]; then
    ...
    SCRIPT_DIR_GLOBAL_CENTER="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    source "${SCRIPT_DIR_GLOBAL_CENTER}/../_eval_metric_helpers.sh"
    unset SCRIPT_DIR_GLOBAL_CENTER
fi

global_center_eval_images() {
    local sample_base=$1
    local step=$2
    local log=$3
    local eval_gpu=$4
