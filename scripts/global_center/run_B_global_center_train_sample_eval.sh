#!/bin/bash
#
# ProMoE-TC-B with global-batch routing-contrastive class centers (engineering
# ablation).  Trains from step 0 to 500K, sampling and evaluating at 300K and
...
cd "$REPO_ROOT"
source "${SCRIPT_DIR}/global_center_eval_helpers.sh"
source "${REPO_ROOT}/scripts/_python_env.sh"

CONFIG="configs/004_ProMoE_B_global_center.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_global_center_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
...
sample_and_eval_step() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU_IDS}" "$PYTHON" sample.py \
        ...
        >> "$LOG" 2>&1
    global_center_eval_images "$SAMPLE_BASE" "$step" "$LOG" "$EVAL_GPU" \
