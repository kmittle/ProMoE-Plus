#!/bin/bash
#
# Train ProMoE-TC-REPA-CROSS-PROTO-B, then sample and evaluate FID.
#
# Experiment 4: Naive REPA + Prototype similarity cross-alignment.
# No additional learnable modules -- cross-alignment weights are derived
# from the existing routing cosine similarities (outer product of each
# token's similarity to its assigned prototype).
#
# Step 1: Train with configs/004_ProMoE_B_repa_cross_proto.yaml
...
CONFIG="configs/004_ProMoE_B_repa_cross_proto.yaml"
LOG="log_ProMoE_B_repa_cross_proto_train_sample_eval.log"
