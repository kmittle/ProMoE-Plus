#!/bin/bash
#
# Strict fresh Base train + sample + eval pipeline.
# The training process records a clean, pushed source/config/environment
# contract in every checkpoint so the routing audit can verify provenance.
...
CONFIG="configs/004_ProMoE_B_fresh_routing_audit_s0_v2.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_fresh_routing_audit_s0_v2_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
