#!/bin/bash
#
# Strict fresh Base seed-1 train + sample + eval pipeline.
# This is an independent clean rerun after the previous pane was interrupted.
...
CONFIG="configs/004_ProMoE_B_seed1_control_v2.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_seed1_control_v2_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
...
PY
    ...
    echo "============================================================" | tee -a "$LOG"
    echo "Phase $((i + 1))/${NUM_ALL_STEPS}: Train to step ${step} (num_steps=${TARGET_NUM_STEPS})" | tee -a "$LOG"
    echo "============================================================" | tee -a "$LOG"

    set +e
    CUDA_VISIBLE_DEVICES="${GPU_IDS}" "$PYTHON" train.py \
        --config "${TEMP_CONFIG}" >> "$LOG" 2>&1
    TRAIN_RC=$?
    set -e
    if [ $TRAIN_RC -ne 0 ]; then
        echo "Training FAILED at phase $((i + 1)) (exit code $TRAIN_RC)" | tee -a "$LOG"
        exit $TRAIN_RC
    fi
    echo "Phase $((i + 1)) training completed successfully" | tee -a "$LOG"

    echo "============================================================" | tee -a "$LOG"
    echo "Phase $((i + 1))/${NUM_ALL_STEPS}: Sample+eval step ${step}" | tee -a "$LOG"
    echo "============================================================" | tee -a "$LOG"
