#!/bin/bash
# Clean seed-0 Base ProMoE control for the DINO routing comparison.
...
CONFIG="configs/004_ProMoE_B_fresh_control_s0_v3.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_fresh_control_s0_v3_train_sample_eval.log"
mkdir -p "$(dirname "${LOG}")"
...
custom_cfg_name = os.path.splitext(os.path.basename(sys.argv[1]))[0]
...
print(",".join(map(str, cfg.get("step_list_for_sample", []))))
...
if [ -z "${STEP_LIST_STR}" ]; then
    echo "ERROR: step_list_for_sample is empty in ${CONFIG}" >&2
    exit 1
fi
...
sample_and_eval_step() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PYTHON}" sample.py \
        --config "${CONFIG}" --step_list_for_sample "${step}" >> "${LOG}" 2>&1
    if [ -d "${SAMPLE_BASE}" ]; then
        while IFS= read -r IMG_DIR; do
            echo "[$(date '+%H:%M:%S')] Evaluating: ${IMG_DIR}" | tee -a "${LOG}"
            (cd evaluation && CUDA_VISIBLE_DEVICES="${EVAL_GPU}" "${PYTHON_EVAL}" \
                run_eval.py "${IMG_DIR}" --count "${NUM_FID_SAMPLES}") >> "${LOG}" 2>&1
