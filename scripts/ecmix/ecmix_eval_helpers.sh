...
# Evaluate one checkpoint's generated images for an EC-mixing run.  The configs
# request CFG 1.0 and CFG 1.5, so a finished step must expose exactly one image
# directory for each setting and a complete evaluator record for both.
if [[ -n "${REPO_ROOT:-}" && -f "${REPO_ROOT}/scripts/_eval_metric_helpers.sh" ]]; then
    source "${REPO_ROOT}/scripts/_eval_metric_helpers.sh"
else
    SCRIPT_DIR_ECMIX="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    source "${SCRIPT_DIR_ECMIX}/../_eval_metric_helpers.sh"
    unset SCRIPT_DIR_ECMIX
fi
