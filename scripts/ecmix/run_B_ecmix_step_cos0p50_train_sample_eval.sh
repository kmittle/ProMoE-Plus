#!/bin/bash
#
# ProMoE-TC-B EC mixing — series 2 (whole steps), EC-step share cosine-annealed 0.50 -> 0 at 500K.
# Trains from step 0 to 500K, then samples and evaluates only the 500K
# checkpoint (this batch has no 300K gate or evaluation).  An interrupted run
...
cd "$REPO_ROOT"
source "${SCRIPT_DIR}/ecmix_eval_helpers.sh"
source "${REPO_ROOT}/scripts/_python_env.sh"

CONFIG="configs/004_ProMoE_B_ecmix_step_cos0p50.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_ecmix_step_cos0p50_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
...
sample_and_eval_step() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU_IDS}" "$PYTHON" sample.py \
        ...
        >> "$LOG" 2>&1
    ecmix_eval_images "$SAMPLE_BASE" "$step" "$LOG" "$EVAL_GPU" \
