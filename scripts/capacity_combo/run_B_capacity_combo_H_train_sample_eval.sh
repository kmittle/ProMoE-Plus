#!/bin/bash
#
# ProMoE-TC-B capacity-aware expert responsibility ablation (H).
...
CONFIG="configs/004_ProMoE_B_capacity_combo_H.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_capacity_combo_H_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"

PYTHON="${PROMOE_TRAIN_PYTHON}"
PYTHON_EVAL="${PROMOE_EVAL_PYTHON}"
[[ -x "$PYTHON" ]] || { echo "ERROR: required training interpreter is missing: $PYTHON" >&2; exit 1; }
[[ -x "$PYTHON_EVAL" ]] || { echo "ERROR: required evaluation interpreter is missing: $PYTHON_EVAL" >&2; exit 1; }
