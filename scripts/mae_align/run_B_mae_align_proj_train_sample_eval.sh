#!/bin/bash
#
# Train ProMoE-TC-B with MAE-style alignment (projector variant), then sample and evaluate FID.
#
# MAE alignment with projectors: each image gets two noise levels (low/high);
# adaLN-conditioned projectors transform high-noise block features to predict
# low-noise features, aligned via negative cosine similarity (coefficient from YAML).
#
# Step 1: Train with configs/004_ProMoE_B_mae_align_proj.yaml via train_with_mae.py
...
CONFIG="configs/004_ProMoE_B_mae_align_proj.yaml"
LOG="log_ProMoE_B_mae_align_proj_train_sample_eval.log"
...
CUDA_VISIBLE_DEVICES="${GPU_IDS}" /mnt/workspace/yujie/.conda/envs/promoe/bin/python train_with_mae.py \
  --config "${CONFIG}" \
  2>&1 | tee -a "$LOG"
