#!/bin/bash
#
# Train ProMoE-TC-B with MAE-style alignment, then sample and evaluate FID.
#
# MAE alignment: each image gets two noise levels (low/high); block features
# are aligned via negative cosine similarity (coefficient from YAML).
#
# Step 1: Train with configs/004_ProMoE_B_mae_align.yaml via train_with_mae.py
...
CONFIG="configs/004_ProMoE_B_mae_align.yaml"
LOG="log_ProMoE_B_mae_align_train_sample_eval.log"
