...
CONFIG="configs/004_ProMoE_B_adepth_q0p1.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_adepth_q0p1_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
...
PY
    ...
    if [[ "$step" == "300000" && "$phase" -lt "$NUM_ALL_STEPS" ]]; then
        gate_rc=0
        adepth_check_300k_gate "$SAMPLE_BASE" "$step" "$LOG" || gate_rc=$?
