#!/bin/bash
#
# Train ProMoE-TC-REPA-MoS-CROSS-GLOBAL-BLOCK-B, then sample and evaluate FID.
#
# Experiment 6: MoS multi-block alignment + per-block BlockAlignAttention cross-alignment.
# A 1-layer transformer computes an attention map at each alignment block's output,
# capturing richer semantic information than pre-block attention (experiment 5).
# Cross-alignment weights combine the attention map with expert grouping masks.
#
# Step 1: Train with configs/004_ProMoE_B_repa_MoS_naive_choice_cross_global_block.yaml
...
CONFIG="configs/004_ProMoE_B_repa_MoS_naive_choice_cross_global_block.yaml"
LOG="log_ProMoE_B_repa_MoS_naive_choice_cross_global_block_train_sample_eval.log"
