#!/bin/bash
#
# Train ProMoE-TC-REPA-MoS-Choice-PerBlock-B (align blocks 3-5), then sample and evaluate FID.
#
# Per-block MoS routing: each aligned DiT block has its own single-layer
# transformer router that runs AFTER the block output to predict per-token
# routing weights over teacher encoder blocks.
#
# Step 1: Train with configs/004_ProMoE_B_repa_MoS_choice_per_block_b3_5.yaml
...
CONFIG="configs/004_ProMoE_B_repa_MoS_choice_per_block_b3_5.yaml"
LOG="log_ProMoE_B_repa_MoS_choice_per_block_b3_5_train_sample_eval.log"
