#!/bin/bash
#
# Train ProMoE-TC-REPA-MoS-CROSS-EXPERT-LOCAL-B, then sample and evaluate FID.
#
# Experiment 7: MoS multi-block alignment + expert-local masked attention cross-alignment.
# A 1-layer transformer computes attention at each alignment block's output, but
# attention is restricted to tokens within the same conditional expert group.
# Cross-alignment weights combine the expert-local attention with expert grouping masks.
#
# Step 1: Train with configs/004_ProMoE_B_repa_MoS_naive_choice_cross_expert_local.yaml
...
CONFIG="configs/004_ProMoE_B_repa_MoS_naive_choice_cross_expert_local.yaml"
LOG="log_ProMoE_B_repa_MoS_naive_choice_cross_expert_local_train_sample_eval.log"
