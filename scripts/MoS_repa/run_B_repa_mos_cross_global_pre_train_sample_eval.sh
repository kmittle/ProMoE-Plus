#!/bin/bash
#
# Train ProMoE-TC-REPA-MoS-CROSS-GLOBAL-PRE-B, then sample and evaluate FID.
#
# Experiment 5: MoS multi-block alignment + GlobalPreAttention cross-alignment.
# A 2-layer transformer computes a global attention map once after patchify,
# which is reused at all alignment blocks. Cross-alignment weights combine
# the attention map with expert grouping masks.
#
# Step 1: Train with configs/004_ProMoE_B_repa_MoS_naive_choice_cross_global_pre.yaml
...
CONFIG="configs/004_ProMoE_B_repa_MoS_naive_choice_cross_global_pre.yaml"
LOG="log_ProMoE_B_repa_MoS_naive_choice_cross_global_pre_train_sample_eval.log"
