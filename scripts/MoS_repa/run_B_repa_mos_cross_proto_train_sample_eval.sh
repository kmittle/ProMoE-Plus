#!/bin/bash
#
# Train ProMoE-TC-REPA-MoS-CROSS-PROTO-B, then sample and evaluate FID.
#
# Experiment 8: MoS multi-block alignment + prototype cosine similarity cross-alignment.
# No new learnable modules — directly reuses the cosine similarity between each token
# and its assigned prototype from the MoE router. Cross-alignment weight between
# token i and j = proto_sim(i) * proto_sim(j) when same expert, 0 otherwise.
#
# Step 1: Train with configs/004_ProMoE_B_repa_MoS_naive_choice_cross_proto.yaml
...
CONFIG="configs/004_ProMoE_B_repa_MoS_naive_choice_cross_proto.yaml"
LOG="log_ProMoE_B_repa_MoS_naive_choice_cross_proto_train_sample_eval.log"
