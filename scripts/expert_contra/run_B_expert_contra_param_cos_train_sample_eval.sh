#!/bin/bash
#
# ProMoE-TC-B expert parameter contrastive — OFAT variant: swap the metric
# from L2 repulsion exp(-dist/tau) to gentle cosine repulsion mean(relu(cos))
# (vs _param baseline). Train + sample + eval, sequential template pipeline.
#
...
LOG="${REPO_ROOT}/logs/log_ProMoE_B_expert_contra_param_cos_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
