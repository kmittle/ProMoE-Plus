#!/bin/bash
#
# Train ProMoE-TC-B with expert output contrastive loss, then sample and evaluate FID.
#
# Adds output-based expert representation contrastive loss on top of routing
# contrastive loss. Each conditional expert's output tokens are mean-pooled,
# then pairwise L2 repulsion loss pushes different experts' representations apart.
#
# Step 1: Train with configs/004_ProMoE_B_expert_contra_output.yaml
...
CONFIG="configs/004_ProMoE_B_expert_contra_output.yaml"
LOG="log_ProMoE_B_expert_contra_output_train_sample_eval.log"
