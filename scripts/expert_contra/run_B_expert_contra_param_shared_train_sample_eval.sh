#!/bin/bash
#
# ProMoE-TC-B expert parameter contrastive — OFAT variant: add the shared
# expert to the pairwise repulsion set, 12 routed + shared (vs _param
# baseline). Train + sample + eval, sequential template pipeline.
#
...
CONFIG="configs/004_ProMoE_B_expert_contra_param_shared.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_expert_contra_param_shared_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
