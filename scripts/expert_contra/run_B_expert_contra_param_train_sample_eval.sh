#!/bin/bash
#
# Train ProMoE-TC-B with expert parameter contrastive loss, then sample and evaluate FID.
#
# Adds parameter-based expert representation contrastive loss on top of routing
# contrastive loss. Each conditional expert's MLP parameters are flattened and
# concatenated, then pairwise L2 repulsion loss pushes different experts' parameter
# vectors apart.
#
# Step 1: Train with configs/004_ProMoE_B_expert_contra_param.yaml
...
CONFIG="configs/004_ProMoE_B_expert_contra_param.yaml"
LOG="log_ProMoE_B_expert_contra_param_train_sample_eval.log"
