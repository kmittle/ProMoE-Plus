...
# tau 0.7 -> 0.07 (10x smaller) on the L2 pairwise repulsion (vs _param
# baseline). Tests whether exp(-dist/tau) is even deader at low tau given the
# ~39 init expert-pair distance. Train + sample + eval, sequential template.
#
...
CONFIG="configs/004_ProMoE_B_expert_contra_param_tau0p07.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_expert_contra_param_tau0p07_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
