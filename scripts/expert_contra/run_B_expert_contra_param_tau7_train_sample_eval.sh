#!/bin/bash
#
# ProMoE-TC-B expert parameter contrastive — OFAT variant: RBF bandwidth
# tau 0.7 -> 7 (10x larger) on the L2 pairwise repulsion (vs _param baseline).
# Tests whether raising tau toward the ~39 init expert-pair distance wakes the
# otherwise-dead exp(-dist/tau) term. Train + sample + eval, sequential template.
#
...
cd "$REPO_ROOT"
source "${SCRIPT_DIR}/expert_contra_eval_helpers.sh"
source "${REPO_ROOT}/scripts/_python_env.sh"

CONFIG="configs/004_ProMoE_B_expert_contra_param_tau7.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_expert_contra_param_tau7_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
...
# A completed 4-GPU reservation can be selected at launch time without
# changing the experiment config or its output identity.
...
sample_and_eval_step() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU_IDS}" $PYTHON sample.py \
       ...
       >> "$LOG" 2>&1
    expert_contra_eval_step "$SAMPLE_BASE" "$step" "$LOG" "$EVAL_GPU" \
...
for i in "${!ALL_STEPS[@]}"; do
    ...
    # The first phase must be a fresh run; only the later phase resumes it.
