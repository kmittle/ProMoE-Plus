...
EXPERT_BASELINE_FID_CFG1="30.584602064850174"
EXPERT_BASELINE_FID_CFG15="9.588081719517504"

has_complete_eval_step() {
    local output_dir=$1
    local step=$2
    local file image_parent image_dir cfg step_dir
    local cfg1_count=0
    local cfg15_count=0
    local files=()
...
has_complete_eval() {
    ...
    # A failed 300K gate is a valid endpoint; a passing gate must also have a
    # complete 500K evaluation before the queue advances.
    has_complete_eval_step "$output_dir" 300000 || return 1
    ...
    promoe_eval_file_metrics_valid "$f1" || return 1
    promoe_eval_file_metrics_valid "$f15" || return 1
    fid1="$(promoe_eval_file_fid "$f1")"
    fid15="$(promoe_eval_file_fid "$f15")"
    if awk -v a="$fid1" -v b="$EXPERT_BASELINE_FID_CFG1" \
        -v c="$fid15" -v d="$EXPERT_BASELINE_FID_CFG15" \
        'BEGIN { exit !(a < b && c < d) }'; then
        has_complete_eval_step "$output_dir" 500000 || return 1
