#!/bin/bash
#
# ProMoE-TC-B with LS-Reg diag(+1) at 0.05 and the pooled-expert cosine regularizer at
# lam=0.1.  One arm of a 2x4 grid (LS-Reg on/off x four lam values).  Trains
...
CONFIG="configs/004_ProMoE_B_dualreg_ls0p05_lam0p1.yaml"
LOG="${REPO_ROOT}/logs/log_ProMoE_B_dualreg_ls0p05_lam0p1_train_sample_eval.log"
mkdir -p "$(dirname "$LOG")"
