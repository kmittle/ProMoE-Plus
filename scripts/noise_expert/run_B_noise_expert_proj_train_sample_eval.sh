#!/bin/bash
#
# Train ProMoE-TC-B with noise expert + AlignProjector, then sample and evaluate FID.
#
# Compared to the base noise expert, this variant adds an adaLN-conditioned MLP
# (AlignProjector) per MoE block. The projector takes noise expert output and
# is conditioned on (t_low, t_high) timestep embeddings to predict the shared
# expert output, then aligns via negative cosine similarity.
#
# Step 1: Train with configs/004_ProMoE_B_noise_expert_proj.yaml
...
CONFIG="configs/004_ProMoE_B_noise_expert_proj.yaml"
LOG="log_ProMoE_B_noise_expert_proj_train_sample_eval.log"
