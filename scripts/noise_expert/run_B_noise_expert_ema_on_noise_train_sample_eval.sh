#!/bin/bash
#
# Train ProMoE-TC-B with noise expert EMA (noise on noise expert input),
# then sample and evaluate FID.
#
# The noise expert is initialized from the shared expert and updated via EMA
# (decay=0.999). Extra noise is added to the noise expert's input, and an
# AlignProjector maps the noised output to predict the shared expert's clean
# output (negative cosine similarity alignment loss).
#
# Step 1: Train with configs/004_ProMoE_B_noise_expert_ema_on_noise.yaml
...
CONFIG="configs/004_ProMoE_B_noise_expert_ema_on_noise.yaml"
LOG="log_ProMoE_B_noise_expert_ema_on_noise_train_sample_eval.log"
