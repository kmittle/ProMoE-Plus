#!/bin/bash
#
# Train ProMoE-TC-B with noise expert EMA (noise on shared expert input),
# then sample and evaluate FID.
#
# The noise expert is initialized from the shared expert and updated via EMA
# (decay=0.999). Extra noise is added to the shared expert's input, and an
# AlignProjector maps the noised output to predict the noise expert's clean
# output (negative cosine similarity alignment loss). This variant trains
# the shared expert to be robust to noisy inputs.
#
# Step 1: Train with configs/004_ProMoE_B_noise_expert_ema_on_shared.yaml
...
CONFIG="configs/004_ProMoE_B_noise_expert_ema_on_shared.yaml"
LOG="log_ProMoE_B_noise_expert_ema_on_shared_train_sample_eval.log"
...
CUDA_VISIBLE_DEVICES="${GPU_IDS}" /mnt/workspace/yujie/.conda/envs/promoe/bin/python train.py \
  --config "${CONFIG}" \
  2>&1 | tee -a "$LOG"
