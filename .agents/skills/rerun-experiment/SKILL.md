...
- every `scripts/_run_times/<date>/<desc>.sh` whose `exec bash` target is one of those semantic scripts;
- each adjacent `<desc>-describe.txt`, when present.
...
Require at least one dated run-time wrapper across the resolved source and any compatible versioned chain before re-bucketing because this workflow must preserve an existing wrapper. If none exists, explain that an in-place rerun is impossible, read [`../new-experiment/SKILL.md`](../new-experiment/SKILL.md) completely, and, after the user accepts a new wrapper and supplies or approves the date and GPU count, follow its missing-runtime rerun handoff. Do not rename any file before that handoff.
...
Determine partial versus complete execution state from config, semantic script, and run-time wrapper only. Treat a missing or stale description as a repairable generated sidecar: it never advances the suffix or changes a wrapper and is regenerated after the target wrapper validates, regardless of whether the output has run.
...
- run-time wrapper: `<desc>.sh` to `<desc>_vN.sh`, preserving any slot prefix a pre-2026-09-23 wrapper still carries;
- description: `<desc>-describe.txt` to `<desc>_vN-describe.txt`, when it exists.
...
Keep the wrapper header and the top-level config `gpu_ids` unchanged. Do not call `scripts/_run_times/new_run.sh` and do not create another wrapper.
...
- unchanged GPU count and `gpu_ids`;
- new output directory;
- validation results;
- the new launch command, without executing it.

For a resumed or idempotently completed chain, report its current artifacts, validation state, and launch command explicitly, and state that no additional rename or wrapper creation occurred.

Use the project's tmux form for the handoff:

```bash
tmux new-window -t "$(tmux display-message -p '#S')" -n <name> \
  'bash scripts/_run_times/<date>/<desc>_vN.sh'
```

## Boundaries

- Rename, edit, validate, and describe only. Never launch a run.
- Never create a new wrapper or change the experiment's GPU count.
...
- Never keep copied old and new run-time wrappers for the same experiment. For a rerun after model-code changes where the user explicitly requires both definitions to remain launchable, read [`../new-experiment/SKILL.md`](../new-experiment/SKILL.md) completely and hand off through its preserve-old rerun mode. That workflow must keep the old files untouched and use `scripts/_run_times/new_run.sh` to give the additional launcher its own wrapper; parallel seeds and equivalent independent copies instead use the independent-parallel-run handoff defined above.
- Every handoff to `$new-experiment` must pass the resolved config, date, GPU count, and the semantic script when present; when it is absent, pass the intended family and training entrypoint so the missing-runtime mode can create it. Since 2026-09-23 a wrapper takes any GPU count and claims idle GPUs at launch; a pre-2026-09-23 wrapper carries a hard-coded slot and `gpu_ids`, so give it a fresh date and wrapper rather than reusing its manual packing.
