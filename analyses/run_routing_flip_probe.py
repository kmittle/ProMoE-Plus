"""Audit whether ProMoE routing follows horizontally flipped content."""
...
from analyses.routing_translation import run_routing_flip_probe
...
def _parse_float_list(value):
    try:
        ...
        raise argparse.ArgumentTypeError(
            "sigmas must be comma-separated floats"
        ) from error
    if not values:
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Horizontally flip a latent and paired noise, then compare native "
            "routes with fixed-compute content-follow and position-follow routes."
        )
    )
