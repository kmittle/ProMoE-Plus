...
from analyses.heatmap.expert_sampling import (
    load_partial_token_choice_expert_result,
    merge_token_choice_expert_partials,
    sample_and_collect_token_choice_expert_heatmaps,
    save_partial_token_choice_expert_result,
)
from analyses.heatmap.plotting import save_class_token_choice_expert_heatmap_svg
from analyses.heatmap.sample_specs import (
    build_heatmap_sample_specs,
    build_token_choice_expert_heatmap_output_stem,
...
def _prepare_runtime_arguments(args):
    ...
    output_dir = resolve_heatmap_output_dir_from_image_dir(image_dir, analysis_name="expert_act")
    ...
    if not selected_class_ids:
        raise ValueError("No ImageNet classes were selected for analysis.")
    ...
    output_stem = build_token_choice_expert_heatmap_output_stem(
        class_ids=selected_class_ids,
        seed=args.seed,
...
def _write_metadata(runtime_args, args) -> None:
    ...
    metadata = {
        ...
        "heatmap_value": "mean_expert_index",
        "topk_reduction": "arithmetic_mean",
        "output_svg_paths": {
...
def _worker(local_rank: int, worker_args: dict) -> None:
    ...
    if not assigned_sample_specs:
        logging.info("No classes were assigned to this worker.")
        return
    ...
    capture = TokenChoiceExpertIndexCapture(model)
    logging.info(
        "Worker %d will analyze %d classes across MoE blocks %s.",
        local_rank,
    ...
    try:
        for sample_batch in chunked(assigned_sample_specs, worker_args["sample_batch_size"]):
            logging.info(
                "Analyzing class batch: %s",
                [sample_spec.class_id for sample_spec in sample_batch],
            )
    ...
    merged_partial = merge_token_choice_expert_partials(partial_results)
    partial_path = Path(worker_args["partial_dir"]) / f"worker_{local_rank:02d}.pt"
    save_partial_token_choice_expert_result(partial_path, merged_partial)


def main():
    parser = argparse.ArgumentParser(
        description=(
            "Visualize token-choice expert routing as RGB heatmaps over final sampled images."
        )
    )
    parser.add_argument(
        "--image-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Capture routed expert indices every N denoising steps.",
    )
    parser.add_argument(
        "--sample-batch-size",
        type=int,
        default=4,
        help="How many classes each worker processes together in one sampling batch.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    partials = [
        load_partial_token_choice_expert_result(partial_path)
        for partial_path in partial_paths
    ]
    merged = merge_token_choice_expert_partials(partials)

    for class_id in runtime_args["selected_class_ids"]:
        class_records = [record for record in merged["samples"] if record["class_id"] == class_id]
        if len(class_records) != 1:
            raise RuntimeError(
                f"Expected exactly one sample record for class {class_id}, got {len(class_records)}."
            )
        save_class_token_choice_expert_heatmap_svg(
            output_path=runtime_args["class_output_paths"][class_id],
            class_id=class_id,
            class_name=runtime_args["class_names"][class_id],
            sample_record=class_records[0],
            analysis_steps=runtime_args["analysis_steps"],
            block_indices=merged["block_indices"],
            max_expert_index=float(merged["max_num_routed_experts"] - 1),
        )

    metadata_path = Path(runtime_args["output_dir"]) / f"{runtime_args['output_stem']}_metadata.yaml"
    with open(metadata_path, "r") as file:
        metadata = yaml.safe_load(file)
    metadata["capture_block_indices"] = [int(block_idx) for block_idx in merged["block_indices"]]
    metadata["num_moe_blocks"] = len(merged["block_indices"])
    metadata["num_routed_experts_per_block"] = {
        int(block_idx): int(num_experts)
        for block_idx, num_experts in merged["num_routed_experts_per_block"].items()
    }
    metadata["token_grid_size"] = int(merged["token_grid_size"])
    metadata["mean_step_count"] = int(merged["mean_step_count"])
    metadata["max_num_routed_experts"] = int(merged["max_num_routed_experts"])
    with open(metadata_path, "w") as file:
        yaml.safe_dump(metadata, file, sort_keys=False, allow_unicode=True)
