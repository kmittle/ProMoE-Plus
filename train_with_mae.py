...
def worker(gpu, cfg):
    ...
    # MAE alignment coefficient (from YAML, default 0.8)
    mae_align_coeff = getattr(cfg, 'mae_align_coeff', 0.8)
    logging.info(f"MAE alignment coefficient: {mae_align_coeff}")
    ...
    # DataLoader loads half batch; we duplicate each image with two noise levels
    loader_batch_size = cfg.train_batch_size // 2
    ...
    steps_per_epoch = total_images // loader_batch_size
    if total_images % loader_batch_size != 0:
        steps_per_epoch += 1
    logging.info(f"----------------------Image Num {total_images} , Total number of steps per epoch: {steps_per_epoch // cfg.world_size}")
    logging.info(f"DataLoader batch_size (half): {loader_batch_size}, effective batch_size per GPU: {cfg.train_batch_size}")
    ...
    while step < cfg.num_steps:
        ...
        half_bs = len(rank_img_y)  # N/2 per GPU
        ...
        # Duplicate latents: [z1, ..., zN/2, z1, ..., zN/2]
        rank_img_z = rank_img_z.repeat(2, 1, 1, 1, 1)
        ...
        if model_pred_raw.shape[1] != noised_z_in.shape[1]:
            model_pred, _ = model_pred_raw.chunk(2, dim=1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train MoE with MAE-style alignment')
