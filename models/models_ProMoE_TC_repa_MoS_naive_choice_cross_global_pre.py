...
class RouterTransformerBlock(nn.Module):
    ...
    Used by both BlockRouter and GlobalPreAttention.
    """
...
#################################################################################
#                       GlobalPreAttention Module                              #
#################################################################################
class GlobalPreAttention(nn.Module):
    """
    2-layer transformer + QK projection, outputs a global attention map.
    Called once after patchify (before DiT blocks), reused at all alignment blocks.
    """
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + MoS REPA + GlobalPreAttention cross-alignment.
    Combines BlockRouter (selects teacher block depth) with GlobalPreAttention
    (determines cross-alignment weights over teacher token positions).
    """
    def __init__(
        self,
        input_size=32,
        ...
        # MoS REPA + GlobalPreAttention cross-alignment
        if repa_config is not None:
            ...
            # GlobalPreAttention for cross-alignment (computed once, reused at all align blocks)
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with MoS REPA + GlobalPreAttention cross-alignment.
        ...
        # Compute global pre-block attention map (once, reused at all align blocks)
        cross_attn_map = None
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.training and routing_weights is not None and cross_attn_map is not None and i in self.align_block_to_idx:
