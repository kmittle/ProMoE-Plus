...
# ============================================================================
# ProMoE-TC Anchor variant — R3-VAE reference-vector removal before routing.
#
# Self-contained copy of models_ProMoE_TC.py; ALL changes are inside
# SparseMoeBlock. Each MoE block holds a learnable anchor r. Cond tokens are
# orthogonally projected off r before the prototype cosine-similarity:
#     alpha = (x . r) / (||r||^2 + eps);  x_new = x - alpha * r   (R3-VAE Eq 1-2)
# Two modes via MoE_config.anchor_apply_mode:
#   "routing" : x_new feeds ONLY the routing cos_sim (experts/shared see x).
#   "replace" : cond tokens are replaced by x_new (router + routed experts +
#               shared_expert all see x_new; uncond tokens untouched; no add-back
#               of alpha*r — the DiTBlock residual skip carries the original x).
#
# NOTE: the anchor is active from step 0 (random init, no zero-gate), so step-0
# is intentionally NOT identical to base ProMoE. This variant is trained fresh
# from scratch (the new `anchor` Parameter makes base-ProMoE checkpoints
# non-strict-loadable).
# ============================================================================
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if anchor_apply_mode not in ("routing", "replace"):
            raise ValueError(
                f"Unsupported anchor_apply_mode: {anchor_apply_mode!r} "
                "(expected 'routing' or 'replace')"
            )
        self.anchor_apply_mode = anchor_apply_mode
        self.anchor_eps = anchor_eps
        # R3-VAE reference vector r — one per MoE block (active from step 0).
        self.anchor = nn.Parameter(torch.empty(hidden_size))

        self._init_weights()

    def _remove_anchor(self, flat_input, cond_mask=None):
        """R3-VAE reference-vector removal (orthogonal projection, NOT cosine):
            alpha = (x . r) / (||r||^2 + eps);  x_new = x - alpha * r

        cond_mask=None  -> rows are already all-cond (routing mode): transform all.
        cond_mask (N,)  -> full flattened tensor (replace mode): keep uncond rows
                           (mask False) unchanged. Out-of-place / autograd-safe; r
                           cast to x dtype for bf16-autocast safety.
        """
        r = self.anchor.to(flat_input.dtype)
        denom = r.pow(2).sum() + self.anchor_eps
        alpha = (flat_input @ r) / denom
        x_new = flat_input - alpha.unsqueeze(1) * r
        if cond_mask is None:
            return x_new
        return torch.where(cond_mask.unsqueeze(1), x_new, flat_input)
    ...
    def compute_router(self, hidden_states, labels):
        ...
        if cond_mask.any():
            ...
            if self.anchor_apply_mode == "routing":
                cond_input = self._remove_anchor(cond_input)
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ...
        if self.anchor_apply_mode == "replace":
            flat_labels = labels.view(batch_size, 1).expand(-1, seq_len).reshape(-1)
            if self.use_uncond_expert:
                cond_mask_fwd = (flat_labels != 1000)
            else:
                cond_mask_fwd = torch.ones(
                    batch_size * seq_len, dtype=torch.bool, device=hidden_states.device
                )
            hidden_states = self._remove_anchor(
                hidden_states.view(-1, hidden_dim), cond_mask_fwd
    ...
    def _init_weights(self):
        ...
        nn.init.normal_(self.anchor, mean=0.0, std=0.02)
