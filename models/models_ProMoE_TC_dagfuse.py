...
# ============================================================================
# ProMoE-TC DAG-Fuse variant — DAG-MoE-style directional fusion between the
# shared-expert output (S, global) and the routed/conditional aggregate (C, local).
#
# Motivation: under top-1 routing a cond token has a single routed expert, so a
# DAG over the *selected experts* is degenerate. Instead we apply DAG-MoE's
# structural aggregation (arXiv 2606.01062, Eq. 7-11) to the 2-node set {C, S},
# letting one role refine itself by reading the other. Conditional aggregation is
# UNCHANGED; only the shared<->conditional combine is replaced (cond tokens only;
# uncond tokens keep the untouched C + S). Shared is always added back (fusion
# completes, never replaces). The up-projection is zero-init, so the block output
# at step 0 is bit-identical to base ProMoE_TC.
#
# Config (MoE_config): fusion_arm in {none, cond_from_shared, shared_from_cond,
# bidirectional} selects the query set Q (the key set is always {C, S}, self-edge
# included); fusion_dim (d_g, default 64) and fusion_num_iter (L, default 1) size
# the lightweight module. All three arms share ONE DAGFuseModule => the three-arm
# ablation is parameter-identical (direction is the only variable).
# ============================================================================
...
class FusedRMSNorm(nn.Module):
    ...
    Used inside DAGFuseModule so the per-iteration normalization never reduces
    in bf16 under autocast."""
...
class DAGFuseModule(nn.Module):
    """DAG-MoE structural aggregation (paper Eq. 7-11) specialized to the K=2 node
    set {C, S}: node 0 = conditional/routed aggregate (local), node 1 = shared
    expert output (global). One src/tgt-factorized gated edge operator per
    iteration; the up-projection is zero-initialized so the module outputs 0 at
    init (step-0 identity: the block output stays C + S).

    `arm` selects the query set Q (which node(s) keep their per-iteration update);
    the key set is ALWAYS {C, S} (self-edge included), so the three arms are
    parameter-identical and differ only in direction:
        cond_from_shared : Q = {C}    (C reads {C, S}; S unchanged)
        shared_from_cond : Q = {S}    (S reads {C, S}; C unchanged)
        bidirectional    : Q = {C, S} (both updated in parallel from pre-update X)
    """
    def __init__(self, hidden_size, dag_dim, num_iter, arm):
        super().__init__()
        assert arm in ("cond_from_shared", "shared_from_cond", "bidirectional"), arm
        assert num_iter >= 1, num_iter
        self.arm = arm
        self.num_iter = num_iter
        self.norms = nn.ModuleList(
            [FusedRMSNorm(hidden_size) for _ in range(num_iter)]
        )
        self.down_projs = nn.ModuleList(
            [nn.Linear(hidden_size, dag_dim, bias=False) for _ in range(num_iter)]
        )
        self.combined_projs = nn.ModuleList(
            [nn.Linear(dag_dim, 4 * dag_dim, bias=False) for _ in range(num_iter)]
        )
        # Zero-init up-projectors. Named '*projectors' so utils.TrainingMonitor's
        # nn.ModuleList rule auto-hooks their grad norms. NOTE: DiT.initialize_weights
        # re-applies this zero-init AFTER its blanket xavier self.apply(_basic_init).
        self.fusion_up_projectors = nn.ModuleList(
            [nn.Linear(dag_dim, hidden_size, bias=False) for _ in range(num_iter)]
        )
        for up in self.fusion_up_projectors:
            nn.init.zeros_(up.weight)
        # Same activation as the expert FFN (MoeMLP uses GELU-tanh), per the paper's
        # "sigma = the same activation function as in the expert FFN".
        self.act = nn.GELU(approximate="tanh")

    def forward(self, c_tokens, s_tokens):
        # c_tokens, s_tokens: [N, d] (same dtype). Returns (C_new, S_new), each [N, d].
        x = torch.stack([c_tokens, s_tokens], dim=1)            # [N, 2, d]; 0=C, 1=S
        for i in range(self.num_iter):
            residual = x
            xn = self.norms[i](x)                               # [N, 2, d]
            xd = self.down_projs[i](xn)                         # [N, 2, d_g]
            comb = self.combined_projs[i](xd)                   # [N, 2, 4*d_g]
            g_src, g_tgt, v_src, v_tgt = comb.chunk(4, dim=-1)  # each [N, 2, d_g]
            # Factorized edge: act(g_src[i] + g_tgt[j]) == sigma(W_edge . concat(x_i, x_j)).
            # axis -3 = query node i (kept), axis -2 = key node j (summed over).
            gate = self.act(g_src.unsqueeze(-2) + g_tgt.unsqueeze(-3))  # [N,2(i),2(j),d_g]
            value = v_src.unsqueeze(-2) + v_tgt.unsqueeze(-3)          # [N,2(i),2(j),d_g]
            msg = gate * value                                  # [N, 2, 2, d_g]
            agg = msg.sum(dim=-2)                               # sum over keys j -> [N,2,d_g]
            upd = self.fusion_up_projectors[i](agg)            # [N, 2, d] (== 0 at init)
            x = upd + residual                                  # [N, 2, d]  (internal residual)
            if self.arm == "cond_from_shared":
                x = torch.stack([x[:, 0], residual[:, 1]], dim=1)   # only C keeps its update
            elif self.arm == "shared_from_cond":
                x = torch.stack([residual[:, 0], x[:, 1]], dim=1)   # only S keeps its update
            # bidirectional: keep x (both nodes updated)
        return x[:, 0], x[:, 1]
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ---- DAG-MoE-style shared<->conditional directional fusion ----
        if fusion_arm not in ("none", "cond_from_shared", "shared_from_cond", "bidirectional"):
            raise ValueError(
                f"Unsupported fusion_arm: {fusion_arm!r} (expected one of 'none', "
                "'cond_from_shared', 'shared_from_cond', 'bidirectional')"
            )
        self.fusion_arm = fusion_arm
        if fusion_arm != "none":
            assert use_shared_expert, "fusion_arm requires a shared expert (node S)"
            self.dag_fuse = DAGFuseModule(
                hidden_size=hidden_size,
                dag_dim=fusion_dim,
                num_iter=fusion_num_iter,
                arm=fusion_arm,
            )
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ...
        ### process shared experts (+ optional directional C<->S fusion)
        if self.use_shared_expert:
            shared_output = self.shared_expert(hidden_states)
            if self.fusion_arm == "none":
                final_output = final_output + shared_output
            else:
                final_output = self._fuse_shared_cond(final_output, shared_output, labels)
    ...
    def _fuse_shared_cond(self, cond_output, shared_output, labels):
        """Directional DAG fusion between the routed/conditional output C and the
        shared output S, on COND tokens only; uncond tokens keep C + S. Returns
        [B, S, d]. Node 0 = C (local/routed), node 1 = S (global/shared); the arm
        on self.dag_fuse decides which node(s) keep their update."""
        batch_size, seq_len, hidden_dim = cond_output.shape
        C_flat = cond_output.reshape(-1, hidden_dim)
        S_flat = shared_output.reshape(-1, hidden_dim)
        out_flat = C_flat + S_flat
        flat_labels = labels.view(batch_size, 1).expand(-1, seq_len).reshape(-1)
        if self.use_uncond_expert:
            cond_mask = (flat_labels != 1000)
        else:
            cond_mask = torch.ones(
                batch_size * seq_len, dtype=torch.bool, device=cond_output.device
            )
        cond_pos = torch.where(cond_mask)[0]
        if cond_pos.numel() > 0:
            Cc = C_flat[cond_pos].float()
            Sc = S_flat[cond_pos].float()
            Cc_new, Sc_new = self.dag_fuse(Cc, Sc)
            fused = (Cc_new + Sc_new).to(out_flat.dtype)
            out_flat = out_flat.index_copy(0, cond_pos, fused)
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # Re-assert DAGFuseModule up-projector zero-init AFTER the blanket
        # self.apply(_basic_init) above (which xavier-overwrites every nn.Linear).
        # Zero up-proj => the fusion module outputs 0 at init => block output == C + S,
        # i.e. step-0 identical to base ProMoE_TC. Mirrors the proto_t re-init pattern.
        for block in self.blocks:
            if getattr(block, "use_moe", False) and getattr(block.mlp, "fusion_arm", "none") != "none":
                for up in block.mlp.dag_fuse.fusion_up_projectors:
                    nn.init.zeros_(up.weight)
