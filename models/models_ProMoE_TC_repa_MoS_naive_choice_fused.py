...
#################################################################################
#          Shared Loss Coefficient Predictor (Transformer + Sigmoid)           #
#################################################################################
class CoeffPredictor(nn.Module):
    """
    Transformer-based loss coefficient predictor for fused MoS REPA.

    Takes the projected student features (z_proj) and conditioning as input,
    and produces per-token sigmoid coefficients for the alignment loss weight.
    Shared across all aligned DiT blocks.

    Inputs:
        z_proj: (N, T, D_teacher) projected student features
        c: (N, D) conditioning embedding (timestep + class)

    Outputs:
        coeff: (N, T) sigmoid loss coefficient
    """
    def __init__(
        self,
        z_dim,
        ...
        super().__init__()
        if router_hidden_dim is None:
            router_hidden_dim = z_dim
        self.router_hidden_dim = router_hidden_dim

        # Input projections
        self.x_norm = nn.LayerNorm(z_dim, eps=1e-6)
        self.x_proj = nn.Linear(z_dim, router_hidden_dim, bias=False)
        ...
        # Output head: per-token scalar coefficient
        self.coeff_head = nn.Linear(router_hidden_dim, 1, bias=False)

    def forward(self, z_proj, c):
        """
        Args:
            z_proj: (N, T, D_teacher) projected student features
            c: (N, D) conditioning (timestep + class embedding)
        ...
        Returns:
            coeff: (N, T) sigmoid coefficients
        """
        N, T, _ = z_proj.shape

        # Project inputs to router hidden dim
        x_proj = self.x_proj(self.x_norm(z_proj))          # (N, T, router_dim)
        c_proj = self.c_proj(self.c_norm(c)).unsqueeze(1)   # (N, 1, router_dim)
        ...
        # Project to coefficient logit and apply sigmoid
        logits = self.coeff_head(h).squeeze(-1)  # (N, T)
        coeff = torch.sigmoid(logits)  # (N, T)
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + Fused MoS REPA alignment.
    Uses a global transformer-based block router to produce per-token routing
    weights over teacher blocks. Top-k teacher features are fused (weighted sum)
    before computing cosine similarity with the projected student features.
    A shared Transformer-based CoeffPredictor predicts per-token sigmoid loss
    coefficients from the projected student features and conditioning.
    """
    def __init__(
        self,
        input_size=32,
        ...
        # Fused MoS REPA: block router + per-block projectors + shared coeff predictor
        if repa_config is not None:
            ...
            print(f"Fused MoS REPA align_blocks ({num_align_blocks}/{depth}): {align_blocks}")
            ...
            # Per-block projectors
            self.mos_projectors = nn.ModuleList([
                build_repa_projector(hidden_size, projector_dim, z_dim) for _ in range(num_align_blocks)
            ])

            # Shared loss coefficient predictor (Transformer + sigmoid)
            coeff_hidden_dim = repa_config.get('coeff_hidden_dim', router_hidden_dim)
            coeff_num_blocks = repa_config.get('coeff_num_blocks', num_router_blocks)
            coeff_num_heads = repa_config.get('coeff_num_heads', router_num_heads)
            self.coeff_predictor = CoeffPredictor(
                z_dim=z_dim,
                cond_size=hidden_size,
                router_hidden_dim=coeff_hidden_dim,
                num_router_blocks=coeff_num_blocks,
                num_heads=coeff_num_heads,
                qk_norm=qk_norm,
            )
        else:
    ...
    def compute_fused_mos_repa_loss(self, x, block_idx, align_idx, routing_weights, teacher_all_z, c, N, T, D):
        """
        Compute fused MoS REPA loss for a single DiT block.

        1. Project student features via per-block projector.
        2. Select top-k teacher blocks using routing weights, fuse them
           via weighted sum (no re-normalization).
        3. Compute cosine similarity between projected student and fused teacher.
        4. Predict per-token loss coefficient via shared CoeffPredictor (sigmoid).
        ...
        Returns:
            block_loss: scalar fused MoS REPA loss for this block
        """
        ...
        # Fuse top-k teacher features using routing weights (no re-normalization)
        # teacher_all_z: (m, N, T, D_teacher) → (N, T, m, D_teacher)
        teacher_permuted = teacher_all_z.permute(1, 2, 0, 3)  # (N, T, m, D_teacher)
        select_idx_expanded = select_idx.unsqueeze(-1).expand(N, T, top_k, teacher_permuted.shape[-1])
        selected_teacher = torch.gather(teacher_permuted, dim=2, index=select_idx_expanded)  # (N, T, top_k, D_teacher)
        fused_teacher = (selected_weights.unsqueeze(-1) * selected_teacher).sum(dim=2)  # (N, T, D_teacher)

        # Cosine similarity between projected student and fused teacher
        z_proj_norm = F.normalize(z_proj, dim=-1)              # (N, T, D_teacher)
        fused_teacher_norm = F.normalize(fused_teacher, dim=-1) # (N, T, D_teacher)
        cos_sim = (z_proj_norm * fused_teacher_norm).sum(dim=-1)  # (N, T)

        # Predict per-token loss coefficient via shared CoeffPredictor
        coeff = self.coeff_predictor(z_proj, c)  # (N, T)

        # Weighted negative cosine similarity
        block_loss = -(cos_sim * coeff).mean()  # scalar
        return block_loss

    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with fused MoS REPA alignment.
        ...
        for i, block in enumerate(self.blocks):
            ...
            # Compute fused MoS REPA loss only for selected blocks
            if self.training and routing_weights is not None and i in self.align_block_to_idx:
                align_idx = self.align_block_to_idx[i]
                block_loss = self.compute_fused_mos_repa_loss(
                    x, i, align_idx, routing_weights, teacher_all_z, c, N, T, D
                )
