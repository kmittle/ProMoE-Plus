...
class DiT(nn.Module):
    ...
    def forward(self, x, timestep, context, **kwargs):
        ...
        Returns (training):
            ...
            zs_proj: list of (N, T, z_dim) projected features (from routed expert output, all tokens)
        Returns (eval):
            x: (N, out_channels, H, W) model prediction
        """
        y = context
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.training and self.projectors is not None and (i + 1) == self.encoder_depth:
                ...
                zs_proj = [proj(routed_output.reshape(-1, D)).reshape(N, T, -1) for proj in self.projectors]
