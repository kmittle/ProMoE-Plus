    ...
    def forward(self, x_embed, c):
        ...
        Returns:
            routing_weights: (N, T, m, n) with softmax over m (teacher blocks)
                             Block-wise: all T tokens share the same (m, n) weights per sample.
        """
        N, T, _ = x_embed.shape
        ...
        # Block-wise routing: mean-pool across tokens before routing head
        h_pooled = h.mean(dim=1)  # (N, router_dim)

        # Project to routing logits and reshape
        logits = self.routing_head(h_pooled)  # (N, m * n)
        logits = logits.reshape(N, self.num_teacher_blocks, self.depth)

        # Softmax over teacher blocks (m) for each generation block (n)
        routing_weights = F.softmax(logits, dim=1)  # (N, m, n)

        # Expand to (N, T, m, n) so all tokens share the same weights
        routing_weights = routing_weights.unsqueeze(1).expand(N, T, self.num_teacher_blocks, self.depth)
