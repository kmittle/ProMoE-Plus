...
class AlignProjector(nn.Module):
    ...
    followed by a two-layer MLP. Projects noised expert features to predict
    clean expert features.
    """
    ...
    def forward(self, feat_noised, t_low, t_high):
        """
        feat_noised: (B, T, D) output features from the expert that received noised input
        t_low:       (B, D)    timestep embedding of low-noise (original clean) images
        t_high:      (B, D)    timestep embedding of noise coefficient
        Returns:     (B, T, D) projected features (prediction of clean expert features)
        """
        t_cond = torch.cat([t_low, t_high], dim=-1)            # (B, 2D)
        shift, scale = self.adaLN_modulation(t_cond).chunk(2, dim=-1)  # each (B, D)
        out = self.norm(feat_noised) * (1 + scale.unsqueeze(1)) + shift.unsqueeze(1)
        out = self.mlp(out)
        return out
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Noise expert: EMA copy of shared expert, receives noised or clean tokens
        # depending on noise_target setting
        ...
        self.noise_target = noise_target  # "noise_expert" or "shared_expert"
        if use_noise_expert:
            assert use_shared_expert, "Noise expert requires shared expert to be enabled"
            assert noise_target in ("noise_expert", "shared_expert"), \
                f"noise_target must be 'noise_expert' or 'shared_expert', got '{noise_target}'"
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor,
        ...
        ### noise expert alignment loss (with AlignProjector + EMA)
        if self.training and self.use_noise_expert:
            if clean_mask is not None and clean_mask.any():
                ...
                if self.noise_target == "noise_expert":
                    # Exp A: noise -> noise_expert, shared_expert output is target
                    noise_expert_out = self.noise_expert(noised_tokens)
                    projected = self.align_projector(noise_expert_out, t_low, t_high)
                    target = shared_output[clean_mask]  # grad flows to shared_expert
                else:
                    # Exp B: noise -> shared_expert, noise_expert output is target
                    shared_out_noised = self.shared_expert(noised_tokens)
                    projected = self.align_projector(shared_out_noised, t_low, t_high)
                    target = self.noise_expert(clean_tokens).detach()  # stable EMA reference

                # Negative cosine similarity loss
                cos_sim = F.cosine_similarity(projected, target, dim=-1)  # [num_clean, seq_len]
                ...
                # Dummy forward for DDP gradient sync (align projector + noise_t_embedder)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.noise_expert_ema_decay = MoE_config.get('noise_expert_ema_decay', 0.999)
        ...
        # After all weights are initialized, copy shared_expert -> noise_expert
        # and freeze noise_expert (it will be EMA-updated, not gradient-updated)
        if self.use_noise_expert:
            self._init_noise_expert_from_shared()

    def _init_noise_expert_from_shared(self):
        """Copy shared expert parameters to noise expert and freeze noise expert."""
        for block in self.blocks:
            if block.use_moe and block.mlp.use_noise_expert:
                block.mlp.noise_expert.load_state_dict(
                    block.mlp.shared_expert.state_dict()
                )
                for p in block.mlp.noise_expert.parameters():
                    p.requires_grad = False

    @torch.no_grad()
    def update_noise_expert_ema(self, decay=None):
        """Update noise expert parameters as EMA of shared expert parameters."""
        if decay is None:
            decay = self.noise_expert_ema_decay
        for block in self.blocks:
            if block.use_moe and block.mlp.use_noise_expert:
                for shared_p, noise_p in zip(
                    block.mlp.shared_expert.parameters(),
                    block.mlp.noise_expert.parameters()
                ):
                    noise_p.data.mul_(decay).add_(shared_p.data, alpha=1 - decay)
