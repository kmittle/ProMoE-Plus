...
class SparseMoeBlock(nn.Module):
    ...
    def compute_router(self, hidden_states, labels):
        ...
        # Cache routing info for cross-alignment
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + cross-alignment via
    prototype cosine similarity. No additional learnable modules beyond
    the standard REPA projector -- cross-alignment weights are derived
    from the existing routing cosine similarities.
    """
    def __init__(
        self,
        input_size=32,
        ...
        # REPA: projector heads for representation alignment (no additional modules)
    ...
    def compute_cross_align_loss(self, z_proj, teacher_z, expert_indices, labels, cross_weights):
        ...
        _build_proto_cross_weights, but retained for interface consistency
        with experiments 1-3.
        ...
        Args:
            ...
            labels: (N,) class labels
            cross_weights: (N, T, T) cross-alignment weights
        Returns:
            loss: scalar
        """
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with cross-alignment via prototype similarity.
