...
# ============================================================================
# ProMoE-TC Prototype-Choice Contrastive variant
#
# Self-contained copy of models_ProMoE_TC.py; the ONLY change vs base is the
# routing-contrastive loss — switched from token-choice to prototype-choice.
# Routing / experts / shared_expert / forward dispatch are UNCHANGED.
#
#   base (token-choice): each token routes to a cluster; a cluster's positive
#     is the mean of the tokens routed to it; negatives are other clusters' means.
#   here (prototype-choice): each prototype selects its top-K most cos-similar
#     cond tokens; the POSITIVE is the MEAN of those top-K; the NEGATIVES are the
#     remaining cond tokens (per-prototype InfoNCE, averaged over prototypes; no
#     prototype-prototype term). K = ceil(contrastive_proto_choice_ratio * N_cond).
#     Every prototype always contributes (no empty-cluster skipping).
# ============================================================================
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        contrastive_proto_choice_ratio=0.1,
        **kwargs,
    ):
        super().__init__()
        ...
        self.contrastive_proto_choice_ratio = contrastive_proto_choice_ratio
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ...
        if self.training and self.routing_contrastive_lam > 0:
            ...
            routing_contrastive_loss = self.compute_proto_choice_contrastive_loss(cond_token_embeddings)
    ...
    def compute_proto_choice_contrastive_loss(self, token_embeddings):
        """Prototype-choice routing-contrastive loss.

        Each prototype (cluster center) selects its top-K most cos-similar cond
        tokens; the POSITIVE is the mean of those top-K, the NEGATIVES are the
        remaining cond tokens (per-prototype InfoNCE, averaged over prototypes;
        no prototype-prototype term). K = ceil(ratio * N_cond).
            token_embeddings: [N, hidden_size]  (cond-token embeddings)
        """
        cluster_centers = self.cluster_centers
        device = cluster_centers.device
        P = cluster_centers.size(0)
        N = token_embeddings.size(0)
        if N < 2:
            return torch.tensor(0.0, device=device)

        K = max(1, min(N - 1, math.ceil(self.contrastive_proto_choice_ratio * N)))
        temperature = self.routing_contrastive_temperature

        centers_norm = F.normalize(cluster_centers, p=2, dim=1)        # [P, D]
        tokens_norm = F.normalize(token_embeddings, p=2, dim=1)        # [N, D]
        sim = centers_norm @ tokens_norm.T                            # [P, N] cos(prototype, token)

        topk_idx = sim.topk(K, dim=1).indices                         # [P, K]
        proto_means = token_embeddings[topk_idx].mean(dim=1)          # [P, D] positive = mean of top-K
        means_norm = F.normalize(proto_means, p=2, dim=1)             # [P, D]
        pos = (centers_norm * means_norm).sum(dim=1) / temperature    # [P]

        neg_mask = torch.ones(P, N, dtype=torch.bool, device=device)
        neg_mask.scatter_(1, topk_idx, False)                         # negatives = tokens NOT in p's top-K
        neg_logits = (sim / temperature).masked_fill(~neg_mask, float('-inf'))  # [P, N]

        # cast to fp32 before cat/logsumexp (pos is fp32 via elementwise mul,
        # neg_logits may be bf16 via matmul under autocast — unify + stabilize)
        pos = pos.float()
        neg_logits = neg_logits.float()
        all_logits = torch.cat([pos.unsqueeze(1), neg_logits], dim=1)  # [P, 1+N]
        loss = (torch.logsumexp(all_logits, dim=1) - pos).mean()
        return loss
