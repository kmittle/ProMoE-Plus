...
#################################################################################
#                          ProMoE_EC_BC_hetero Layer                             #
# Heterogeneous-expert variant of ProMoE_EC_BC (batch-choice Expert-Choice).     #
#                                                                                #
# Motivation: TC + heterogeneous experts (models_ProMoE_TC_hetero_expert) cannot #
# keep FLOPs fair — a token-choice router is free to send more tokens to the     #
# wide (3x) experts to chase performance, so total compute drifts and is only    #
# softly bounded by a cost penalty. Expert-Choice fixes this structurally: every #
# expert has a FIXED capacity k = (T / E) * top_k and picks its own top-k tokens #
# from the batch-flattened cond pool (T = B_cond * S), so per-expert token count  #
# is a constant independent of routing. Total FLOPs = k * sum_i(cost_i) is then   #
# a fixed structural value the router cannot inflate.                             #
#                                                                                #
# Routed experts use linearly increasing intermediate sizes (1x -> 3x, same      #
# ladder as models_ProMoE_TC_hetero_expert); the uncond expert stays at          #
# moe_intermediate_size (2x). Because the linear ladder averages to 2x, total     #
# FLOPs EXACTLY MATCH homogeneous ProMoE_EC_BC — this is a FLOPs-matched control. #
# No cost penalty is used: EC hard-caps per-expert token count, so the soft       #
# FLOPs regularizer of the TC hetero-expert variant is unnecessary here.          #
#                                                                                #
# All other settings (routing direction, index_add_ dispatch, uncond handling,   #
# shared expert, batch-choice routing contrastive loss) match ProMoE_EC_BC.       #
# Expert shapes differ from base ProMoE => trained fresh (base checkpoints are    #
# non-strict-loadable), same as models_ProMoE_TC_hetero_expert.                   #
#################################################################################
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Routed experts: heterogeneous middle_dim, linearly from 1d to 3d.
        # (Same ladder as models_ProMoE_TC_hetero_expert; the mean is 2d so the
        #  total FLOPs exactly match homogeneous ProMoE_EC_BC under EC's fixed
        #  per-expert capacity.)
        ...
        experts_list = [
            MoeMLP(hidden_size=hidden_size, intermediate_size=expert_intermediate_sizes[i])
            for i in range(num_routed_experts)
        ]
        # Unconditional expert: 2d middle_dim (same as moe_intermediate_size)
        if use_uncond_expert:
            experts_list.append(
    ...
    def compute_router(self, cond_hidden_states):
        """
        Batch-Choice routing: each expert picks its top-k tokens from the
        batch-flattened pool of (B_cond * S) cond tokens, instead of from
        each image's S tokens independently.
        ...
        flat_input = cond_hidden_states.reshape(T, D)  # (T, D)

        input_norm = F.normalize(flat_input, p=2, dim=-1)              # (T, D)
        cluster_norm = F.normalize(self.cluster_centers, p=2, dim=-1)  # (E_cond, D)
        # cos_sim: (T, E_cond)
        cos_sim = input_norm @ cluster_norm.T
        cos_sim_expert_view = cos_sim.transpose(0, 1)  # (E_cond, T)
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ...
        if cond_batch_mask.any():
            ...
            ### token assignment (batch-flatten)
            flat_indices, gating_scores, expert_inputs = self.compute_router(cond_hidden_states)
...
#################################################################################
#                       Core ProMoE_EC_BC_hetero Model                           #
#################################################################################
