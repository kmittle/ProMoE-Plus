...
class SparseMoeBlock(nn.Module):
    ...
    def compute_routing_contrastive_loss(self, token_embeddings, cluster_assignments, use_top_k=False):
        ...
        loss_centers2mean = F.cross_entropy(logits, labels)
        loss_mean2centers = F.cross_entropy(logits.T, labels)  # add symmetric loss from mean to centers
        loss = (loss_centers2mean + loss_mean2centers) / 2
