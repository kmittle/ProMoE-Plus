...
# ============================================================================
# ProMoE-TC LB-Contra variant (改进组二) — load-balance-aware routing contrastive loss.
#
# ONLY the routing contrastive auxiliary loss changes; routing / experts / shared /
# forward / params are all UNCHANGED vs base ProMoE_TC (no new nn.Parameter), so the
# forward is bit-identical to base and base checkpoints load strict=True.
#
# The base loss is an M×M InfoNCE over HARD per-cluster token means (empty clusters
# skipped) — it drives routing ACCURACY but is blind to load. Three ablation modes
# (config `lb_contra_mode`) inject the per-codeword assignment count to also pressure
# LOAD BALANCE. All three build one shared per-token SOFT assignment over ALL K
# prototypes (differentiable, per-GPU local): q_t=softmax(cos(token,centers)/τ_route),
# soft counts ñ_i=Σ_t q_t[i] (>0 for every K), soft means m_i=Σ_t q_t[i]·token/ñ_i.
#   reweight     : per-row CE × w_i=(ñ_i)^(-beta)         (欠载权重大)
#   logit_adjust : logits[:,j] += tau_adj·log(ñ_j)        (balanced-softmax 方向)
#   balance_term : CE + lambda·K·Σ_i f_i·P_i              (Switch, f detached)
# a/b use count as a DETACHED weight/bias; c gets gradient via the soft P_i. No STE.
# `none` keeps the base hard-mean InfoNCE; `soft_only` is the soft-mean all-K InfoNCE
# with NO modulation (control isolating the hard->soft mean switch). Config knobs: lb_reweight_beta,
# lb_logit_adj_tau, lb_balance_lambda, lb_route_tau (default = routing_contrastive_temperature).
# ============================================================================
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        lb_contra_mode="none",
        lb_reweight_beta=1.0,
        lb_logit_adj_tau=1.0,
        lb_balance_lambda=0.01,
        lb_route_tau=None,
        lb_count_eps=1e-6,
        **kwargs,
    ):
        ...
        # ---- load-balance-aware routing-contrastive config (改进组二) ----
        assert lb_contra_mode in ("none", "soft_only", "reweight", "logit_adjust", "balance_term"), lb_contra_mode
        self.lb_contra_mode = lb_contra_mode
        self.lb_reweight_beta = lb_reweight_beta
        self.lb_logit_adj_tau = lb_logit_adj_tau
        self.lb_balance_lambda = lb_balance_lambda
        self.lb_count_eps = lb_count_eps
        self.lb_route_tau = lb_route_tau if lb_route_tau is not None else routing_contrastive_temperature
    ...
    def compute_routing_contrastive_loss(self, token_embeddings, cluster_assignments, use_top_k=False):
        """Load-balance-aware routing contrastive loss (改进组二) — dispatches by
        self.lb_contra_mode. `none` keeps the base hard-mean InfoNCE; the other three
        modes build one shared per-token SOFT assignment over ALL K prototypes
        (differentiable, per-GPU local) and inject the soft count ñ_i:
            reweight     : per-row CE × w_i=(ñ_i)^(-beta)   (欠载权重大, ñ detached)
            logit_adjust : logits[:,j] += tau_adj·log(ñ_j)  (ñ detached)
            balance_term : CE.mean() + lambda·K·Σ_i f_i·P_i (Switch, f=P.detach())
        Counts enter as detached modulation (a/b) or via the soft P_i (c); no STE.
        """
        mode = self.lb_contra_mode
        if mode == "none":
            return self._hard_mean_infonce(token_embeddings, cluster_assignments, use_top_k)

        centers = self.cluster_centers                          # [K, D]
        K = centers.size(0)
        device = centers.device
        eps = self.lb_count_eps
        if token_embeddings.size(0) == 0:
            return torch.tensor(0.0, device=device)

        # ---- shared soft-assignment substrate (all K, per-GPU local, differentiable) ----
        tok_norm = F.normalize(token_embeddings, p=2, dim=1)    # [N, D]
        cen_norm = F.normalize(centers, p=2, dim=1)             # [K, D]
        cos_tok = (tok_norm @ cen_norm.T).clamp(-1.0, 1.0)      # [N, K]  (clamp: bf16 safety)
        q = F.softmax(cos_tok / self.lb_route_tau, dim=1)       # [N, K]  soft assignment
        soft_n = q.sum(dim=0)                                   # [K]     soft counts (> 0)
        N = soft_n.sum()                                        # == N_cond
        soft_mean = (q.t() @ token_embeddings) / (soft_n.unsqueeze(1) + eps)   # [K, D]

        # ---- all-K InfoNCE ----
        means_norm = F.normalize(soft_mean, p=2, dim=1)         # [K, D]
        sim = (cen_norm @ means_norm.t()).clamp(-1.0, 1.0)      # [K, K]
        logits = sim / self.routing_contrastive_temperature     # [K, K]
        labels = torch.arange(K, device=device)

        if mode == "soft_only":
            # control: soft-mean all-K InfoNCE with NO count modulation. Isolates the
            # hard->soft mean switch from the load-balance modulation (a/b/c).
            return F.cross_entropy(logits, labels)

        if mode == "logit_adjust":
            bias = self.lb_logit_adj_tau * torch.log(soft_n.detach() + eps)   # [K]
            logits = logits + bias.unsqueeze(0)                 # add to candidate columns j
            return F.cross_entropy(logits, labels)

        ce = F.cross_entropy(logits, labels, reduction="none")  # [K] per-row CE

        if mode == "reweight":
            w = (soft_n.detach() + eps) ** (-self.lb_reweight_beta)           # [K] 欠载权重大
            w = w * (K / w.sum())                               # normalize mean weight -> 1
            return (w * ce).mean()

        # mode == "balance_term"  (Switch-style load balance added to the InfoNCE)
        p = soft_n / N                                          # [K] soft prob (grad)
        f = p.detach()                                          # [K] detached fraction
        balance = K * (f * p).sum()                             # min at uniform
        return ce.mean() + self.lb_balance_lambda * balance

    def _hard_mean_infonce(self, token_embeddings, cluster_assignments, use_top_k=False):
        """
