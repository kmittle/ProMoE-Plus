...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ---- lsreg: per-prototype label smoothing on the routing contrastive loss ----
        # anti-overfitting (NOT load balance); does NOT touch routing — eps is detached and
        # only reshapes the contrastive TARGET; compute_router/forward are byte-identical to base.
        self.ls_mode = ls_mode                 # "off"|"fixed"|"dyn_both"|"dyn_under"|"dyn_over"
        self.ls_eps_base = ls_eps_base
        self.ls_slope = ls_slope
        self.ls_eps_cap = ls_eps_cap
        self.ls_load_map = ls_load_map         # "linear" (batch1) | "invsqrt" (batch2)
        self.ls_ema_beta = ls_ema_beta
        ...
        # broadcast from rank 0 each forward -> ls_load_ema is a rank-0-sourced (not rank-local)
        # load estimate, blended with each rank's current-batch counts. Fine here: data is IID
        # across ranks, eps is DETACHED (no gradient / no DDP grad-reduction impact), and it keeps
        # eps consistent across ranks. (No train.py change needed.)
        self.register_buffer("ls_load_ema", torch.zeros(num_routed_experts), persistent=False)
        self.register_buffer("_ls_step", torch.zeros(1), persistent=False)
        self.last_mean_eps = None              # rank-0 readout hook (train.py logs this)
    ...
    def compute_routing_contrastive_loss(self, token_embeddings, cluster_assignments, use_top_k=False):
        ...
        cluster_means = []
        valid_clusters = []
        full_counts = torch.zeros(num_clusters, device=device)  # lsreg: all-K counts for EMA
        ...
        for cluster_id in range(num_clusters):
            ...
            full_counts[cluster_id] = mask.sum().float()   # lsreg: record every cluster's count
            if mask.sum() > 0:
    ...
    def _compute_ls_eps(self, valid_counts, ema):
        """lsreg: per-prototype smoothing eps from sample-count reliability (detached, in [0, cap])."""
        valid_counts = valid_counts.float()
        cap = self.ls_eps_cap
        if self.ls_mode == "fixed":
            eps = torch.full_like(valid_counts, float(self.ls_eps_base))
        elif self.ls_load_map == "invsqrt":
            n_ref = self.ls_n_ref
            if n_ref is None:
                n_ref = valid_counts.mean()          # reference over VALID rows (empties excluded)
            eps = self.ls_eps_max * torch.clamp(torch.sqrt(n_ref / valid_counts.clamp(min=1e-6)) - 1.0, 0.0, 1.0)
        else:  # linear rel-based (dyn_both / dyn_under / dyn_over)
            n_bar = valid_counts.mean()              # reference over VALID rows (empties excluded)
            rel = ((valid_counts - n_bar) / (n_bar + 1e-6)).clamp(-1.0, 1.0)   # >0 overloaded
            if self.ls_mode == "dyn_under":
                eps = self.ls_eps_base + self.ls_slope * torch.clamp(-rel, min=0.0)
            elif self.ls_mode == "dyn_over":
                eps = self.ls_eps_base + self.ls_slope * torch.clamp(rel, min=0.0)
            else:  # dyn_both
                eps = self.ls_eps_base - self.ls_slope * rel
        if self.ls_warmup and self.ls_warmup > 0:
            w = torch.clamp(self._ls_step.float() / float(self.ls_warmup), 0.0, 1.0)
            eps = eps * w
        return eps.clamp(0.0, cap).detach()
