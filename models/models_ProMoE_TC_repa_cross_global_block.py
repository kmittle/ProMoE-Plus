...
class BlockAlignAttention(nn.Module):
    """
    2-layer transformer + QK projection on block output, outputs attention map.
    Computed at the alignment block's output (after DiT block processing),
    capturing richer semantic information than pre-block attention.
    Single block alignment uses 2 layers; multi-block (MoS) uses 1 layer.
    """
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + cross-alignment via BlockAlignAttention.
    Attention map is computed at the alignment block's output (per-block attention).
    """
    def __init__(
        self,
        input_size=32,
        ...
        if repa_config is not None:
            ...
            # Assert encoder_depth points to a MoE block
            ...
            # BlockAlignAttention for cross-alignment (computed at block output)
    ...
    def compute_cross_align_loss(self, z_proj, teacher_z, expert_indices, labels, cross_weights):
        """
        Cross-alignment loss: weighted negative cosine similarity between student
        projections and teacher features, where weights combine attention map,
        expert grouping mask, and conditional mask.
        ...
        # Conditional mask: exclude unconditional samples
        ...
        # Final weights = attention_weights * same_expert * both_conditional
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with cross-alignment via BlockAlignAttention.
