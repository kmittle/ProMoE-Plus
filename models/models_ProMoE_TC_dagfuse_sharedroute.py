...
# ============================================================================
# ProMoE-TC Shared-Augment (idea 3: dagfuse_sharedroute) — the resource-lean
# version of idea 2: instead of fusing ALL previous MoE shared outputs, a
# per-block router selects the top-k most useful previous MoE blocks PER TOKEN.
#
# Router (Codex review, plan v2 §4): each MoE block owns a lightweight router
# `nn.Linear(d, num_prev_moe)` scoring the `num_prev_moe` earlier MoE blocks from
# the current block's RAW shared token (the query). softmax over the previous
# blocks, top-k selection is done OUTSIDE (torch.topk), and the selected source is
# weighted by its softmax probability — TOP-1 IS ALSO WEIGHTED (probability kept,
# NOT renormalized to 1.0) so the router receives gradient. num_prev_moe varies
# per block, so each block's router is sized from its moe_ordinal; the first MoE
# block (ordinal 0) has no previous block and reduces to plain base ProMoE (no
# augment params).
#
# Cache = RAW shared (same as idea 2; forward-local, not detached). Zero-init
# output projection => step-0 bit-identical to base ProMoE_TC. Routing (the MoE
# expert router) is byte-identical to base — this router only picks augmentation
# sources for the shared output. Config (MoE_config): fuse_apply in {none, cond,
# all}, fuse_mech in {dag, softmax} (idea 3 uses dag), fuse_top_k (1 or 2),
# fuse_dim (d_g, default 64).
# ============================================================================
...
class SharedAugmentModule(nn.Module):
    ...
    def forward(self, target, sources, source_weights=None):
        # target: [N, d]; sources: [N, K, d] (K >= 1). Returns [N, d] = target + zero-init delta.
        # source_weights: optional [N, K] per-source scaling (e.g. the router softmax probs).
        # It is applied to each source's MESSAGE *after* the per-key RMSNorm — NOT to the source
        # vector before it — because RMSNorm(alpha*v) == RMSNorm(v) would otherwise cancel a
        # pre-norm scalar weight, killing the router's gradient path. The self-edge (key 0) is
        # always weight 1.
        ...
        if source_weights is not None:
            ones = source_weights.new_ones(source_weights.shape[0], 1)
            key_w = torch.cat([ones, source_weights], dim=1)     # [N, 1+K]; self-edge weight 1
        xn = self.norm(keys)
        if self.mech == "dag":
            ...
            msg = gate * value                                   # [N, 1+K, d_g]
            if key_w is not None:
                msg = msg * key_w.unsqueeze(-1).to(msg.dtype)    # weight sources POST-norm (uncancelled)
            agg = msg.sum(dim=1)
            ...
            if key_w is not None:
                attn = attn * key_w.to(attn.dtype)               # weight sources POST-norm (uncancelled)
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ---- routed shared-expert augmentation (select top-k previous MoE shared) ----
        ...
        self.moe_ordinal = moe_ordinal            # index among MoE blocks == num previous MoE blocks
        # augment machinery only where there is at least one previous MoE block
        self.has_augment = (fuse_apply != "none" and moe_ordinal > 0)
        if self.has_augment:
            assert use_shared_expert, "fuse_apply requires a shared expert (augment target)"
            self.router = nn.Linear(hidden_size, moe_ordinal)          # per-token score over prev blocks
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor, aug_sources: torch.Tensor = None):
        ...
        ### process shared experts (+ optional routed shared augmentation)
        shared_raw = None
        if self.use_shared_expert:
            shared_output = self.shared_expert(hidden_states)
            shared_raw = shared_output  # RAW shared, cached by DiT.forward (keeps grad)
            if self.has_augment:
                if aug_sources is not None and aug_sources.shape[2] > 0:
                    shared_output = self._augment_shared_routed(shared_output, aug_sources, labels)
                shared_output = self._fake_touch(shared_output)  # keep router/augment params in DDP graph
        ...
        return final_output, loss, shared_raw

    def _augment_shared_routed(self, shared_output, sources, labels):
        """Route + fuse: score the `m` previous MoE shared outputs per token from the
        current raw shared token (query), softmax over them, keep top-k, gather the
        selected same-position source tokens, modulate (per-block aug_mod), weight by
        softmax probability (top-1 also weighted, NOT renormalized), then add a
        zero-init gated delta via SharedAugmentModule. sources: [B, S, m, d]."""
        B, S, d = shared_output.shape
        m = sources.shape[2]
        if m == 0:
            return shared_output
        k = min(self.fuse_top_k, m)
        logits = self.router(shared_output)                       # [B, S, m]
        w = F.softmax(logits, dim=-1)                             # [B, S, m]
        topw, topi = torch.topk(w, k=k, dim=-1)                   # [B, S, k]
        idx = topi.unsqueeze(-1).expand(-1, -1, -1, d)           # [B, S, k, d]
        sel = torch.gather(sources, 2, idx)                       # [B, S, k, d] selected raw shared
        sel = self.aug_mod(sel)                                   # per-block modulation
        # NOTE: the softmax weight `topw` is passed to shared_augment as source_weights and
        # applied to each source's message AFTER the per-key RMSNorm (top-1 kept, not
        # renormalized). Scaling `sel` here (pre-norm) would be cancelled by that RMSNorm.
        tgt = shared_output.reshape(B * S, d)
        src = sel.reshape(B * S, k, d)
        sw = topw.reshape(B * S, k)                               # [B*S, k] router softmax probs
        if self.fuse_apply == "cond" and self.use_uncond_expert:
            ...
            if pos.numel() == 0:
                return shared_output
            aug = self.shared_augment(tgt[pos], src[pos], source_weights=sw[pos])
            out = tgt.index_copy(0, pos, aug.to(tgt.dtype))
        else:
            out = self.shared_augment(tgt, src, source_weights=sw)
        return out.view(B, S, d)

    def _fake_touch(self, x):
        """Add 0 * (sum of router+augment params) so those submodules always receive
        gradient — avoids a DDP unused-parameter crash when a block has no sources
        this pass or the all-uncond batch short-circuits (fuse_apply='cond')."""
        s = x.new_zeros(())
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0,
        ...
        if use_moe:
            if use_swiglu==False:
                ...
                self.mlp = SparseMoeBlock(hidden_size=hidden_size, moe_ordinal=moe_ordinal, **MoE_config)
            else:
                self.mlp = SparseMoeBlock(hidden_size=hidden_size, moe_ordinal=moe_ordinal, **MoE_config)
        else:
            if use_swiglu==False:
    ...
    def forward(self, x, c, label, shared_cache=None):
        # shared_cache: forward-local list of previous MoE blocks' RAW shared outputs.
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        if self.use_moe:
            sources = None
            if shared_cache is not None and len(shared_cache) > 0:
                sources = torch.stack(shared_cache, dim=2)  # [B, S, m, d]
            x_mlp, aux_loss, shared_raw = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp), label, aug_sources=sources)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # moe_ordinal = index among MoE blocks (== number of preceding MoE blocks),
        # used to size each block's source router; -1 for Dense blocks.
        blocks = []
        moe_ord = 0
        for i in range(depth):
            is_moe = use_moe_flag[i]
            blocks.append(DiTBlock(hidden_size, num_heads // (1 if is_moe else 1), head_dim=head_dim,
                                   mlp_ratio=mlp_ratio, qk_norm=qk_norm, use_swiglu=use_swiglu,
                                   MoE_config=MoE_config, use_moe=is_moe,
                                   moe_ordinal=(moe_ord if is_moe else -1)))
            if is_moe:
                moe_ord += 1
    ...
    def forward(self, x, timestep, context, **kwargs):
        ...
        c = t + y                                # (N, D)
        shared_cache = []  # forward-local: previous MoE blocks' RAW shared outputs (kept in graph)
        for block in self.blocks:
            x, shared_raw = block(x, c, labels, shared_cache=shared_cache)  # (N, T, D)
            if shared_raw is not None:
                shared_cache.append(shared_raw)
