...
class DiT(BaseDiT):
    """ProMoE with sparse first-order denoising-regret router preferences."""

    def __init__(self, *args, denoising_regret_config=None, **kwargs):
        super().__init__(*args, **kwargs)

        config = denoising_regret_config or {}
        self.denoising_regret_block = config.get(
            'denoising_regret_block', 3
        )
        self.denoising_regret_probe_interval = config.get(
            'denoising_regret_probe_interval', 16
        )
        self.denoising_regret_token_ratio = config.get(
            'denoising_regret_token_ratio', 0.0625
        )
        self.denoising_regret_candidate_mode = config.get(
            'denoising_regret_candidate_mode', 'mixed'
        )
        self.denoising_regret_confidence_quantile = config.get(
            'denoising_regret_confidence_quantile', 0.5
        )
        self.denoising_regret_temperature = config.get(
            'denoising_regret_temperature', 0.1
        )
        self.denoising_regret_warmup_steps = config.get(
            'denoising_regret_warmup_steps', 10000
        )
        self.denoising_regret_ramp_steps = config.get(
            'denoising_regret_ramp_steps', 10000
        )
        self.denoising_regret_label_roll = config.get(
            'denoising_regret_label_roll', 0
        )
        self.denoising_regret_seed = config.get(
            'denoising_regret_seed', 271828
        )
        self.denoising_regret_eps = config.get(
        ...
        if (
            ...
            raise ValueError(
                "denoising_regret_block must be an integer in "
                f"[0, {len(self.blocks) - 1}], got {block_idx!r}"
            )
        if not self.blocks[block_idx].use_moe:
            raise ValueError(
                f"denoising_regret_block {block_idx} must be a MoE block"
            )
    ...
    def compute_denoising_regret_loss(
        ...
        _, num_tokens, hidden_size = hidden_states.shape
        ...
        if conditional_rows.numel() == 0:
            local_stats = [local_zero.detach() for _ in range(5)]
        ...
        with torch.no_grad():
            router_result = moe_layer.compute_router(hidden_states.detach(), labels)
            route_weights, route_indices = router_result[:2]
    ...
    def forward(
        ...
        **kwargs,
        ...
        try:
            prediction = super().forward(x, timestep, context, **kwargs)
        finally:
            if hook is not None:
                hook.remove()
