...
# ============================================================================
# ProMoE-TC Shared-Augment (idea 2: dagfuse_densenet) — DenseNet-style: augment
# each MoE block's SHARED-expert output from ALL PREVIOUS MoE blocks' shared
# outputs.
#
# Motivation: prior DAG-fusion found augmenting the shared expert is the key
# lever. Idea 2 feeds each MoE block's shared output with the shared outputs of
# every earlier MoE block (a DenseNet-style skip over the shared-expert stream).
#
# Cache semantics (Codex review, plan v2 §1.2 — user decision): the cache holds
# each block's RAW shared output (shared_expert(hidden_states), BEFORE its own
# augmentation), threaded forward-locally through DiT.forward (a plain list, NOT
# a persistent self.* buffer) and NOT detached (gradient flows back into earlier
# shared experts). One SHARED per-block modulation MLP (aug_mod) transforms every
# source; the (1+K)-key SharedAugmentModule then adds a single zero-init gated
# delta. The first MoE block has K=0 sources => degenerates to base (a fake-grad
# touch keeps its augment params in the DDP graph).
#
# Zero-init output projection => step-0 bit-identical to base ProMoE_TC (non-strict
# loadable). Routing is byte-identical to base (fusion touches only the shared
# output). Config (MoE_config): fuse_apply in {none, cond, all} (default none ==
# base), fuse_mech in {dag, softmax} (idea 2 uses dag), fuse_dim (d_g, default 64).
# ============================================================================
...
class SharedAugmentModule(nn.Module):
    ...
    the same key set {target} ∪ sources. The output projection is zero-initialized,
    so at step 0 the delta is exactly 0 and the block output is bit-identical to
    base ProMoE_TC.
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ---- shared-expert augmentation from all previous MoE shared outputs ----
        ...
        if fuse_apply != "none":
            ...
            self.aug_mod = nn.Linear(hidden_size, hidden_size)          # ONE shared per-block source modulation
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor, aug_sources: torch.Tensor = None):
        ...
        ### process shared experts (+ optional DenseNet-style shared augmentation)
        shared_raw = None
        if self.use_shared_expert:
            shared_output = self.shared_expert(hidden_states)
            shared_raw = shared_output  # RAW shared, cached by DiT.forward for later blocks (keeps grad)
    ...
    def _augment_shared(self, shared_output, sources, labels):
        ...
        sources: [B, S, K, d] (raw previous shared outputs; modulated here by the ONE
        per-block aug_mod). fuse_apply='cond' augments only cond tokens (labels !=
        1000); 'all' augments every token. Routing is untouched."""
        B, S, d = shared_output.shape
        K = sources.shape[2]
        if K == 0:
            return shared_output
        tgt = shared_output.reshape(B * S, d)
        src = self.aug_mod(sources.reshape(B * S, K, d))  # ONE shared per-block modulation over all K
    ...
    def _fake_touch(self, x):
        ...
        gradient — avoids a DDP unused-parameter crash for the first MoE block (K=0
        sources) or the rare all-uncond batch (fuse_apply='cond')."""
        s = x.new_zeros(())
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, label, shared_cache=None):
        ...
        # ([B,S,d] each). MoE blocks read all of them (K sources) and return their own
        # raw shared for appending; Dense blocks return (x, None).
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        if self.use_moe:
            sources = None
            if shared_cache is not None and len(shared_cache) > 0:
                sources = torch.stack(shared_cache, dim=2)  # [B, S, K, d]
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # Zero output proj => augment delta 0 at init => step-0 identical to base.
        for block in self.blocks:
