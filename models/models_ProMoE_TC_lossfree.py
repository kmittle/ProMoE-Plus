...
# ============================================================================
# ProMoE-TC Loss-Free Balancing variant (改进组四) — auxiliary-loss-free routing
# load balance, after "Auxiliary-Loss-Free Load Balancing for MoE" (DeepSeek,
# arXiv 2408.15664). Instead of a balance AUX LOSS (which injects interference
# gradients — cf. 改进组二), add a per-prototype BIAS b_i to the cond routing
# score used ONLY for the top-1 SELECTION; the expert output weight uses the
# UNBIASED cos-sim => zero interference gradient into the main/contrastive loss.
# After each training forward, b_i is updated (no grad) from this batch's cond
# load: b_i += u * sign(mean_c - c_i)  (underloaded -> raise, overloaded -> lower),
# with c_i all_reduce'd across ranks (global load). b_i is a BUFFER (not a
# Parameter). STEP-0: b_i init 0 => biased == unbiased => routing == base =>
# forward bit-identical (and NO new trainable params). uncond + contrastive loss
# UNCHANGED. Config: use_lossfree_bias {False default}, bias_update_rate u.
# ============================================================================
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ---- Loss-Free Balancing (改进组四): per-prototype routing bias (buffer, no grad) ----
    ...
    def compute_router(self, hidden_states, labels):
        ...
        if cond_mask.any():
            ...
            if self.use_lossfree_bias:
                # Loss-Free Balancing: SELECT by the biased score, but WEIGHT by the UNBIASED
                # cos-sim -> the bias only reroutes, injecting zero interference gradient.
                sel_score = cond_weights + self.expert_bias.unsqueeze(0)
                topk_idx = torch.topk(sel_score, k=self.top_k, dim=1).indices
        ...
        # ---- Loss-Free Balancing: update per-prototype bias from THIS batch's cond load ----
        # Runs unconditionally when (enabled & training) so all ranks hit the all_reduce
        # collective (avoids DDP deadlock on rare all-uncond batches). No grad; b_i is a buffer.
        if self.use_lossfree_bias and self.training:
            with torch.no_grad():
                counts = torch.zeros(self.num_routed_experts, device=device, dtype=torch.float)
                if cond_mask.any():
                    # per-expert load = each cond token's routed prototype(s). reshape(-1) flattens
                    # the top_k columns -> the correct top-K load count; base is top_k=1 so this is
                    # exactly the top-1 assignment.
                    cond_sel = expert_indices[torch.where(cond_mask)[0]].reshape(-1).to(torch.long)
                    counts.scatter_add_(0, cond_sel, torch.ones_like(cond_sel, dtype=torch.float))
                if dist.is_available() and dist.is_initialized():
                    dist.all_reduce(counts, op=dist.ReduceOp.SUM)     # global (computation-batch) load
                mean_c = counts.sum() / self.num_routed_experts
                self.expert_bias += self.bias_update_rate * torch.sign(mean_c - counts)
