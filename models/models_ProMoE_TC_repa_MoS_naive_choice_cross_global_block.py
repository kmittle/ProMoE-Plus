...
class RouterTransformerBlock(nn.Module):
    ...
    Used by both BlockRouter and BlockAlignAttention.
    """
...
#################################################################################
#                       BlockAlignAttention Module                             #
#################################################################################
class BlockAlignAttention(nn.Module):
    """
    Per-block attention for cross-alignment. Uses 1-layer transformer preprocessing
    + QK projection on block output, producing a (N, T, T) attention map.
    Multi-block (MoS) alignment uses 1 layer (vs 2 layers in single-block naive REPA).
    """
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + MoS REPA + BlockAlignAttention cross-alignment.
    Combines BlockRouter (selects teacher block depth) with per-block BlockAlignAttention
    (determines cross-alignment weights over teacher token positions at each align block).
    """
    def __init__(
        self,
        input_size=32,
        ...
        # MoS REPA + BlockAlignAttention cross-alignment
        if repa_config is not None:
            ...
            # Per-block BlockAlignAttention for cross-alignment
            # Each align block has its own 1-layer attention module
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with MoS REPA + BlockAlignAttention cross-alignment.
        ...
        # No pre-block attention — each align block computes its own attention map
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.training and routing_weights is not None and i in self.align_block_to_idx:
                ...
                # Compute per-block attention map at this block's output.
                ...
                cross_attn_map = self.block_align_attns[align_idx](x.detach())  # (N, T, T)
