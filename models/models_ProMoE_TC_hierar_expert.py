...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer('alpha_penalty', ratios / ratios.sum())
    ...
    def compute_router(self, hidden_states, labels):
        ...
        return router_weights, expert_indices, load_balance_loss, cost_penalty

    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ### token assignment
        router_weights, expert_indices, load_balance_loss, cost_penalty = self.compute_router(hidden_states, labels)
        ...
        ### cost penalty loss
        if self.training and self.cost_penalty_lam > 0 and cost_penalty is not None:
            cost_penalty_loss = cost_penalty * self.cost_penalty_lam
            if loss is not None:
                loss += cost_penalty_loss
            else:
                loss = cost_penalty_loss
