...
class SparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor):
        ...
        # Collect per-expert outputs for output-based expert contrastive loss
        need_expert_outputs = (
            self.training
            and self.compute_expert_contrastive
            and self.expert_contrastive_lam > 0
            and self.expert_contrastive_mode == "output"
        )
        expert_output_pools = {}  # expert_id -> pooled output [hidden_dim]
        ...
        for expert_id in range(self.num_experts):
            ...
            if token_ids.numel() > 0:
                ...
                # Collect pooled output for conditional experts only
        ...
        ### expert representation contrastive loss
    ...
    def _expert_contrastive_output(self, expert_output_pools):
        """
        Output-based expert contrastive loss.
        expert_output_pools: dict mapping expert_id -> pooled output tensor [hidden_dim]
        """
    ...
    def _pairwise_repulsion_loss(self, vecs):
        """
        Compute pairwise repulsion loss: mean(exp(-||v_i - v_j||_2 / temperature))
        over all unique pairs (i, j).
        vecs: [K, D] tensor of K vectors
        """
        K = vecs.size(0)
        if K < 2:
            return torch.tensor(0.0, device=vecs.device)

        # Pairwise L2 distances: [K, K]
        # ||v_i - v_j||_2 = sqrt(sum((v_i - v_j)^2))
        ...
        # Loss: mean(exp(-dist / temperature))
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # Expert contrastive: set flag on selected blocks
        expert_contrastive_blocks = MoE_config.get('expert_contrastive_blocks', [])
        for block_idx in expert_contrastive_blocks:
            assert self.blocks[block_idx].use_moe, \
                f"expert_contrastive_blocks contains block {block_idx} which is not a MoE block"
