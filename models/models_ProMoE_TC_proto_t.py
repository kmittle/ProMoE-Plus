...
#################################################################################
#                              ProMoE-TC proto_t Layer                          #
# Timestep-conditioned prototype routing. The static cluster_centers are        #
# transformed by a PrototypeMLP using each sample's timestep embedding before   #
# being used in routing-similarity computation, so that both sides of cos_sim   #
# live in the same noise-level feature space.                                   #
#################################################################################
...
class SparseMoeBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Timestep-conditioned prototype generator. time_dim == hidden_size because
        # TimestepEmbedder outputs at hidden_size. Step 0 behavior is bit-identical
        # to base ProMoE-TC (residual: 0 delta; direct: identity-init proto_proj).
    ...
    def compute_router(self, hidden_states, labels, prototype_t):
        """
        Args:
            hidden_states: (B, S, D)
            labels: (B,)
            prototype_t: (B, E, D) per-sample timestep-conditioned prototypes
        """
        ...
        if cond_mask.any():
            ...
            # Each cond token must use ITS OWN sample's prototype for similarity.
            # cond_positions are flat indices into (B*S); sample index = pos // seq_len.
            cond_sample_idx = cond_positions // seq_len                  # (n_cond,)
            per_token_proto = prototype_t[cond_sample_idx]               # (n_cond, E, D)

            input_norm = F.normalize(cond_input, p=2, dim=1)             # (n_cond, D)
            proto_norm = F.normalize(per_token_proto, p=2, dim=-1)       # (n_cond, E, D)

            # cos_sim[i, e] = <input_norm[i], proto_norm[i, e]>
            cos_sim = torch.einsum('id,ied->ie', input_norm, proto_norm)
    ...
    def forward(self, hidden_states: torch.Tensor, labels: torch.Tensor, t_emb: torch.Tensor):
        ### timestep-conditioned per-sample prototypes
        prototype_t = self.prototype_mlp(self.cluster_centers, t_emb)  # (B, E, D)
        ### token assignment
        router_weights, expert_indices, load_balance_loss = self.compute_router(hidden_states, labels, prototype_t)
