...
class RouterTransformerBlock(nn.Module):
    """
    A simple pre-norm transformer block.
    Used by both BlockRouter and ExpertLocalAttention.
    """
...
class ExpertLocalAttention(nn.Module):
    """
    Expert-local masked attention for cross-alignment.
    Uses 1-layer transformer preprocessing (multi-block MoS scenario) + QK projection.
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + MoS REPA + ExpertLocalAttention cross-alignment.
    Combines BlockRouter (selects teacher block depth) with per-block ExpertLocalAttention
    (determines cross-alignment weights within expert groups at each align block).
    """
    def __init__(
        self,
        input_size=32,
        ...
        # MoS REPA + ExpertLocalAttention cross-alignment
        if repa_config is not None:
            ...
            # Per-block ExpertLocalAttention for cross-alignment
            # Each align block has its own 1-layer expert-local attention module
            self.expert_local_attns = nn.ModuleList([
                ExpertLocalAttention(
                    hidden_size=hidden_size,
                    num_heads=repa_config.get('align_attn_num_heads', 8),
                    num_blocks=1,   # multi-block (MoS) → 1 layer
    ...
    def _build_expert_local_mask(self, expert_indices, labels, N, T):
        """
        Build expert-local attention mask.
        True where two tokens belong to the same image and same conditional expert.

        Args:
            expert_indices: (N, T, top_k) or None (dense block)
            labels: (N,) class labels
            N, T: batch/token sizes
        Returns:
            mask: (N, T, T) bool
        """
        if expert_indices is None:
            # Dense block: all conditional tokens can attend to each other
            cond_mask = (labels != 1000).unsqueeze(1).expand(-1, T)  # (N, T)
            return cond_mask.unsqueeze(2) & cond_mask.unsqueeze(1)  # (N, T, T)
    ...
    def compute_cross_mos_repa_loss(self, x, align_idx, routing_weights, teacher_all_z,
        ...
        # For ExpertLocalAttention, the attention map already has expert-local masking,
        # but we still apply expert_mask in compute_cross_mos_repa_loss for consistency.
        if expert_indices is not None:
            # MoE block: apply expert grouping mask
            ...
            # Dense block: no expert grouping, all cond tokens interact
            cond_mask = (labels != 1000).unsqueeze(1).expand(-1, T)
            pair_cond = cond_mask.unsqueeze(2) & cond_mask.unsqueeze(1)
        ...
        with torch.no_grad():
            ...
            self._cross_align_stats = {
                'cos_sim_absmax': max(absmax, prev.get('cos_sim_absmax', 0)),
                'cos_sim_clamp_count': clamp_ct + prev.get('cos_sim_clamp_count', 0),
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with MoS REPA + ExpertLocalAttention cross-alignment.
        ...
        # No pre-block attention — each align block computes its own expert-local attention
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.training and routing_weights is not None and i in self.align_block_to_idx:
                ...
                # Build expert-local mask for this block's routing
