...
class GlobalPreAttention(nn.Module):
    ...
    Called once after patchify (before DiT blocks), reused at alignment time.
    """
...
class DiT(nn.Module):
    """
    Diffusion model with a Transformer backbone + cross-alignment via GlobalPreAttention.
    """
    def __init__(
        self,
        input_size=32,
        ...
        if repa_config is not None:
            ...
            # GlobalPreAttention for cross-alignment
    ...
    def forward(self, x, timestep, context, teacher_all_z=None, **kwargs):
        """
        Forward pass of DiT with cross-alignment via GlobalPreAttention.
        ...
        # 1. Compute global pre-block attention map (training only)
        cross_attn_map = None
