...
# 四点组合实验（H/R/O/P）：只跑了 HROP

> **状态（2026-09-14）**：组合方式待重新设计。原计划的 8 臂组合矩阵和后来补的两个 `_norm` 臂里，只启动了 `H+R+O+P`；其余 9 个臂从未启动，已连同配置、脚本、队列，以及只有这些臂或从未启动的草稿才用到的模型开关（归一化输出视角、cosine/联合签名、按宽度加权的 capacity LS-Reg）一起删除。原计划在矩阵之后做的 `H-Rcap` 系列依赖已删除的 capacity LS-Reg 代码，也不再保留。

`H+R+O+P` 使用 `configs/004_ProMoE_B_capacity_combo_HROP.yaml`，seed 0、全局 batch 256、从 step 0 训练，四个训练期改动同时打开，推理计算量不变：

- **H**：routed experts 的中间宽度按 1x--3x 线性排列并向下取整，B 规模下为 `[768, 907, 1047, 1186, 1326, 1466, 1605, 1745, 1885, 2024, 2164, 2304]`，平均宽度与 Base 基本一致。
- **R**：历史 token-count diagonal LS-Reg（强度 0.05，EMA β=0.9），只改路由对比损失。
