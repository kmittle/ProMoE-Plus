...
- `train_with_MoS_repa.py`: MoS-REPA, MoS-Naive / Naive-Choice, separate-projector / per-block / blockwise / fused / multi-align, Teacher-Affinity Routing, shared-routed spectral responsibility, teacher-conditioned expert geometry, and both standard-REPA + MoS cross-alignment training with teacher-block routing and per-block REPA projectors.
...
- `scripts/MoS_repa/`: MoS-REPA, naive / naive-choice, per-block / blockwise / fused, multi-align, Teacher-Affinity Routing, shared-routed spectral responsibility, teacher-conditioned expert geometry, cross-alignment, and B/L/XL block-range sweep wrappers following `scripts/template.sh`.
...
- `train_with_MoS_repa.py` hosts MoS, naive/choice/separate/blockwise/per-block/fused variants, multi-align, teacher-affinity multi-align, shared-routed spectral-responsibility multi-align, teacher-conditioned expert-geometry multi-align, and standard REPA + MoS cross-alignment families.
- `train_with_mae.py` hosts `group_align` and `group_align_proj` families.
- Plain DiT and most ProMoE variants return a tensor. REPA variants return `(pred, zs_proj)` during training. MoS-REPA, multi-align, fused MoS, and cross-alignment variants return `(pred, alignment_loss)` during training. Teacher-affinity, spectral-responsibility, and teacher-conditioned expert-geometry multi-align return `(pred, alignment_loss, auxiliary_loss)`; `train_with_MoS_repa.py` dispatches the third value by registered family and weights it with `teacher_affinity_coeff`, `spectral_responsibility_coeff`, or `expert_geometry_coeff`, respectively.
