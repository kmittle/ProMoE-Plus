...
- `train_with_MoS_repa.py`: MoS-REPA, MoS-Naive / Naive-Choice, separate-projector / per-block / blockwise / fused / multi-align, Teacher-Affinity Routing, shared-routed spectral responsibility, and both standard-REPA + MoS cross-alignment training with teacher-block routing and per-block REPA projectors.
...
- `scripts/MoS_repa/`: MoS-REPA, naive / naive-choice, per-block / blockwise / fused, multi-align, Teacher-Affinity Routing, shared-routed spectral responsibility, cross-alignment, and B/L/XL block-range sweep wrappers following `scripts/template.sh`.
...
- `train_with_MoS_repa.py` hosts MoS, naive/choice/separate/blockwise/per-block/fused variants, multi-align, teacher-affinity multi-align, shared-routed spectral-responsibility multi-align, and standard REPA + MoS cross-alignment families.
- `train_with_mae.py` hosts `group_align` and `group_align_proj` families.
- Plain DiT and most ProMoE variants return a tensor. REPA variants return `(pred, zs_proj)` during training. MoS-REPA, multi-align, fused MoS, and cross-alignment variants return `(pred, alignment_loss)` during training. Teacher-affinity multi-align returns `(pred, alignment_loss, teacher_affinity_loss)` and spectral-responsibility multi-align returns `(pred, alignment_loss, spectral_responsibility_loss)`; `train_with_MoS_repa.py` dispatches the third value by registered family and weights it with the matching top-level coefficient.
